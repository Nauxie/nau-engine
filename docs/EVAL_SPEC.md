...
- `player_pose_sheet.svg`: windowless player fixture pose sheet emitted by `tools/player_pose_preview.sh` for human review, including mesh-surface contact overlays where connector surfaces visibly separate and translucent markers where tracked mesh pairs approach overlap thresholds.
...
- The current pose eval surface proves named state coverage, readable key-pose floors, sampled connector surface distance, projected visual contact gaps, proximal shoulder/hip contact bounds, first-pass player/glider hand-to-grip attachment gates, and a minimum limb-anatomy detail layer. It still does not prove final character art quality; future passes should add richer full-body silhouette metrics, denser hand/finger-to-glider animation checks, and temporal pose-smoothness/video review before substantially increasing model complexity.
