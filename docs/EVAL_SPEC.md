...
The export writes `manifest.json`, `wind_tracks/wind_visual_tracks.obj`, `wind_tracks/wind_visual_tracks.ndjson`, and `audit.json`. The OBJ is a line-segment artifact for visual inspection; the NDJSON records every sampled guide/ribbon center or mesh sample across the `0.0s`, `0.2s`, and `0.4s` coherence window. The audit validates schema, artifact presence, OBJ vertex/segment counts, NDJSON track count, full updraft/crosswind field coverage, expected guide/ribbon counts, non-static motion, bounded visual speed/acceleration, continuity gaps, bounded off-field and low-alignment samples, and recomputed displacement/field-containment/flow-alignment coherence against public `WindField::flow_at` math. This is the background-safe gate for wind visuals being present, moving smoothly, and broadly attached to the same gameplay field model before screenshot review.
