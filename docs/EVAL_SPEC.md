...
- pose-state coverage scenarios record grounded idle/walk/run samples, grounded walk/run stride foot travel and leg opposition, readable launch/fall/glide/air-turn/air-brake/gliding-dive samples, right and left air-turn pose samples, landing anticipation/recovery samples, landing crouch, feet-forward flare, asymmetric foot-split floor, recovery flip, scarf stream/sway/flex floors, and app-authored `idle`/`walk`/`run`/`fall` clip samples while rejecting unreadable key-pose samples
...
- `pose_state_coverage`: current full-pose chain coverage for idle hold, walk/run stride, launch/fall/glide, right and left air-turn, air-brake/gliding-dive, landing anticipation/recovery shape, app-authored idle/walk/run/fall, zero-unreadable pose-state coverage, and bounded transition-grace samples.
