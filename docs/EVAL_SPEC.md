...
- current gates require lateral response within 0.20 seconds, at least 18 m/s directional right/left lateral response, at least 10 m/s rear-right and rear-left lateral response, at least 10 m/s rear-right and rear-left rearward response, at least 12 m/s of total and planar air-brake speed drop, at least 14 m/s of post-brake forward recovery, at least 8 degrees of left and right body-bank response, at least 8 degrees of signed right and left pose lean, at least four readable `air_turn` pose samples with nonzero right and left `air_turn` coverage, at least one visible pose temporal sample, pose-part rotation/translation deltas at or under 120 degrees/0.55 m, sampled body-roll steps at or under 12 degrees, camera view-yaw and world-yaw drift at or under 2 degrees, p95 body-heading error at or under 22 degrees, at least eight desired-travel heading samples with p95/max error at or under 45/65 degrees, at least one right, left, backward-right, and backward-left desired-travel sample, at least one right and left lateral body/travel heading sample plus at least one backward-right and backward-left diagonal body/travel heading sample, lateral body/travel heading p95/max error at or under 45/60 degrees, backward-diagonal body/travel heading p95/max error at or under 35/50 degrees, and max body-heading/yaw-step error at or under 36 degrees
...
- `semantic_scene_audit.json`: projected terrain/foliage/cloud/distant-island material-family and `terrain_surface`/`tree_canopy`/`weather_cloud`/`distant_island` scene-kind hit, terrain material/biome variant diversity, visible terrain material-variant hit coverage, and aggregate pixel-coverage audit for screenshot evals that emit marker sidecars.
...
- The simulation-only binary intentionally skips renderer, screenshots, frame-time, asset-server, generated-asset collision contact, and visual content checks, but it does cover the bounded wind-current response gates. Pair it with app/screenshot/export audits before treating a branch as fully verified.
...
- Island collision follows deterministic authored terrain relief, and obvious generated asset obstacles now use simple AABB world-collision proxies. Runtime evals gate proxy presence, report resolved samples, meaningful contact samples, and push distance, and use `world_collision_contact` to prove the player is pushed by a generated launch-mesa obstacle, but the collision model is still a route-surface clamp plus horizontal proxy push-out rather than full rigid-body physics or production imported colliders.
