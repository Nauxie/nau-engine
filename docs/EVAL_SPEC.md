...
- `player_pose_sheet.svg`: windowless player fixture pose sheet emitted by `tools/player_pose_preview.sh` for human review, including mesh-surface contact overlays where connector surfaces visibly separate and translucent markers where tracked mesh pairs approach overlap thresholds; its manifest also records the mesh-projected silhouette audit used to gate fall/glide/dive outlines.
- `player_transition_pose_sheet.svg`: windowless player fixture transition sheet emitted by `tools/player_pose_preview.sh`, showing high-risk 20%, 40%, 60%, and 80% traversal blend frames with the same mesh-surface contact and overlap overlays.
- `glider_pose_sheet.svg`: windowless glider deployment pose sheet emitted by `tools/player_pose_preview.sh`, using shared per-view scale so stowed, takeout, glide, dive, and air-brake silhouettes can be compared visually.
- `player_glider_attachment_sheet.svg`: windowless combined player/glider pose sheet emitted by `tools/player_pose_preview.sh`, showing takeout, glide, dive, and air-brake attachment rows with hand-to-grip distance overlays for human review.
