...
- `asset_fixture_audit.json`: structural glTF fixture audit run through `tools/eval.sh` unless `NAU_EVAL_ASSET_AUDIT=0` is set, including player mesh-projected silhouette gates, organic major-limb volume gates, and rest-pose worst-overlap reports for the current pose fixture.
...
- The current pose eval surface proves named state coverage, readable key-pose floors, sampled connector surface distance, projected visual contact gaps, proximal shoulder/hip contact bounds, wrist-pivoted hand wiring, major-joint deformation-envelope bounds, organic major-limb volume density floors, fingertip-aware player/glider hand-to-grip attachment gates, a minimum limb-anatomy detail layer, rest-pose worst-overlap reporting, and mesh-projected full-body fall/glide/dive silhouette gates. It still does not prove final character art quality; future passes should add temporal hand-grip video review, pose-smoothness review, and richer rendered sidecars before substantially increasing model complexity.
