...
- pose torso pitch, arm spread, leg tuck, lateral lean, signed right/left lateral lean, landing crouch, landing foot-forward tuck, landing flare, wing-airflow maxima, authored glider response/motion maxima, authored glider dive response/motion maxima, authored bank-left/bank-right/dive/air-brake/landing clip sample counts, key-pose readability min/max, unreadable key-pose sample count, and key-pose transition-grace sample count
