...
- Traversal, route-surface geometry, visual wind-field geometry, gameplay lift math, wind-sway visual motion, diagnostics, eval metrics, and richer pose math live in testable pure functions in `src/lib.rs`, with movement and camera math split into dedicated modules.
...
3. Add a simulation-only eval binary if native-window metric runs become a scaling bottleneck.
