...
- `updraft_route` eval proves a scripted route enters a gameplay lift field, sees a paired visible updraft while lift is active, samples dynamic wind flow with nonzero speed/gust variation/range, applies sustained bounded lateral wind response, gates simultaneous layered crosswind-plus-updraft swirl force overlap, gates updraft/crosswind guide and ribbon motion, per-field guide/ribbon coverage, sustained visual-flow sample windows, depth span, scale pulse, and coherence against shared field flow, and gains altitude only through `LiftField` lift.
...
- Crosswind and updraft airflow still use generated flow-driven guide gizmos/ribbons/motes/haze rather than production particles, cloth simulation, or authored environment effects.
