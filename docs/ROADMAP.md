...
As of 2026-07-06, camera stability supersedes further world/visual expansion. Manual playtests on the authored-world stack found camera stutter, object-proximity jitter, drift, unexpected zoom-out, off-center framing, at least one loss of camera/player control, and a follow-up high-island jump that still zoomed far away after early fixes. The current camera-contract branch now has app-path high-island, obstruction-reset, strafe, frame-time trace, and movement-heavy air-control coverage plus lower render/visual density, but the exact manual failure path still needs foreground validation. PR1/PR2 are merged to `main` after clean verification; do not merge PR3+ of the authored-world stack into `main` until the camera incident is reproduced against `main`/PR boundaries or replaced by equivalent stress coverage, fixed at the root, and covered by app-path stress evals that exercise manual-style orbit/input near dense objects, high-island falls, movement acceleration, and streaming/LOD churn. PR3 through the camera stabilization layer are draft-held because the regression was first isolated at PR3.
