...
- wind-current evals gate sustained updraft visual rise, per-field guide/ribbon coverage, sustained updraft/crosswind visual-flow sample windows, updraft/crosswind visual depth span, baseline-relative guide/ribbon scale pulse, split guide/ribbon crosswind motion along the gameplay field direction, short-horizon guide/ribbon motion aligned with shared soft-edged gust-cell `WindField::flow_at`, and wind-force deltas aligned with the field-axis correction direction toward that same shared flow; sustained visual-flow samples must clear meaningful fractions of the same visual floors instead of merely being nonzero, and focused visual tests also cover stream-specific crosswind variation, sampled-flow-facing crosswind guide orientation, phase-staggered thermal depth lanes, traveling clumped gust packets, and secondary gust energy between primary fronts so motes do not move in lockstep
