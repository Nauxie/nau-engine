...
- pose-intent sample counts for readable gliding, launching, falling, diving, air brake, landing anticipation, and landing recovery; app and simulation summaries also count grounded idle/walk/run pose samples, deployed-glider dive samples, right/left air-turn samples, same-sample pure-sideways AirTurn alignment samples, and right/left plus backward-right/backward-left air-brake pose samples, pose-state checks require both right and left air-turn samples, and app evals score visible generated or authored player part transforms when those nodes are available and still require authored clip alignment when authored player geometry hides generated fallback parts
- pose torso pitch, arm spread, leg tuck, dive-only torso pitch, dive-only arm spread, dive-only leg tuck, lateral lean, signed right/left lateral lean, signed backward-right/backward-left air-brake lateral lean, landing crouch, landing foot-forward tuck, landing foot split, landing flare, bounded landing flare/recovery backbend, wing-airflow maxima, scarf stream/sway/flex maxima, authored glider response/motion maxima, authored glider dive response/motion maxima, authored idle/walk/run/bank-left/bank-right/fall/dive/air-brake/landing clip sample counts, key-pose readability min/max, unreadable key-pose sample count, and key-pose transition-grace sample count, including bounded app grace for short air-brake, glide, dive, landing-absorb, and landing-anticipation release blends; focused pose unit tests also require dive-specific phase-staggered airflow motion across torso, head, arms, and legs while preserving key-pose readability
...
- landing-required routes exercised readable landing-anticipation pose coverage before contact, readable landing-recovery pose coverage after contact, reached at least 0.05 m of landing crouch, reached at least 0.32 m of feet-forward landing tuck, reached at least 56 degrees of landing flare while staying under the max landing-flare backbend, produced zero key-pose samples below the 0.9 readability floor after bounded transition grace, recorded landing-only visible-pose temporal samples, and kept landing pose-part rotation/translation deltas within landing-specific route thresholds
