...
impl SkyRoute {
    ...
    pub fn resolve_grounded_after_horizontal_correction(
        ...
        if (state.position.y - ground.floor_y).abs() <= GROUNDED_SURFACE_FOLLOW_MAX_STEP_M {
    ...
    fn resolve_ground_contact_with_landing(
        ...
        let surface_offset_m = state.position.y - ground.floor_y;
        let touches_surface_from_above =
            surface_offset_m.abs() <= GROUND_CONTACT_EPSILON && state.velocity.y <= 0.0;
        let crosses_surface_downward = (-GROUND_CONTACT_VERTICAL_CAPTURE_M
            ..-GROUND_CONTACT_EPSILON)
            .contains(&surface_offset_m)
            && state.velocity.y < 0.0;
        if touches_surface_from_above || crosses_surface_downward || follows_nearby_surface {
