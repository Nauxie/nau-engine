...
These are acceptable for the current harness. The next meaningful upgrades are asynchronous asset-loading simulation, vegetation/cloud image-readability checks, exact route-marker semantic checks, richer world-semantic projection for screenshot audits, and a simulation-only eval binary if native-window metric runs become a scaling bottleneck.
