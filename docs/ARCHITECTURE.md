...
12. Island terrain is resident inside the active stream window, inactive or non-near chunks show cheap distant impostors, route beacons remain resident for readability, and nonessential island detail is despawned outside the near LOD band.
13. HUD text reports frame time, mode, speed, altitude, camera pitch/distance/framing/motion/orbit alignment, obstruction adjustment, mouse yaw/pitch offsets, velocity, visual asset slots, missing/queued/loading/loaded/failed load-state buckets, asset residency classes, visual wind-field count, lift-field count, active chunk window, near/mid/far LOD island buckets, visible/hidden terrain, impostor, and detail counts, resident island visual count, stream entity churn, route beacons, cooldown, and launch readiness.
14. Debug gizmos draw player vectors, the camera line, visual wind/updraft streams, and gameplay lift-field bounds.
15. The render stack uses Bevy-native atmosphere, dynamic sun/fog/exposure weather, volumetric fog/light, bloom, filmic tonemapping, procedural PBR texture maps, reflective/transmissive water, emissive markers, drifting cloud banks, and high cirrus veils.
...
Eval samples include camera distance, camera surface clearance, camera-to-player framing angle, per-frame camera step and rotation deltas, camera orbit alignment, camera view yaw, obstruction adjustment/hits, camera yaw/pitch offsets, route objective progress, `active_lift_fields`, sky-island count, active chunk count, active island count, near/mid/far LOD island counts, visible/hidden island terrain counts, visible/hidden island impostor counts, visible/hidden island detail counts, visible route beacon count, weather cloud count, resident island visual count, stream entity churn, declared visual asset slot/readiness counts, Bevy scene load-state buckets, asset residency class counts, and entity count. Eval summaries include frame-time avg/p95/p99/max telemetry, `lifted_samples`, objective-progress checks, camera-control/framing/orbit/view-yaw/obstruction/jerk checks, checkpoint screenshot artifact paths, sky-island/content-scale checks, asset-slot inventory and failed-load checks, streaming/LOD planning checks, stream-churn checks, weather-cloud checks, resident visual/churn checks, visible-detail/beacon checks, and scene entity-count checks. Screenshot evals launched through `tools/eval.sh` add a non-golden `visual_audit.json` for image resolution, exposure, contrast, color variety, edge density, per-frame scene coverage, center-scene detail, per-frame player visibility, HUD-text dominance, sequence-level route-marker readability, and sequence-level sky coverage; `updraft_route` verifies gameplay lift and objective progress, `branch_recovery_route` verifies the full branch objective sequence, `long_glide_visibility` verifies larger archipelago traversal and content scale, `camera_mouse_control` verifies mouse X/Y behavior, `camera_yaw_stability` verifies that small yaw input does not drift after input stops, `camera_strafe_stability` verifies that `A`/`D` movement does not auto-orbit the camera, and `camera_turn_stability` verifies rapid airborne turn and air-brake camera stability.
