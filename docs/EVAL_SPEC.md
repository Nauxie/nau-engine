...
- `asset_fixture_audit.json`: structural glTF fixture audit run through `tools/eval.sh` unless `NAU_EVAL_ASSET_AUDIT=0` is set, including player mesh-projected silhouette gates for the current pose fixture.
...
- The current pose eval surface proves named state coverage, readable key-pose floors, sampled connector surface distance, projected visual contact gaps, proximal shoulder/hip contact bounds, wrist-pivoted hand wiring, major-joint deformation-envelope bounds, fingertip-aware player/glider hand-to-grip attachment gates, a minimum limb-anatomy detail layer, and mesh-projected full-body fall/glide/dive silhouette gates. It still does not prove final character art quality; future passes should add temporal hand-grip video review, pose-smoothness review, and richer rendered sidecars before substantially increasing model complexity.
