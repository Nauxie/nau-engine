...
- `./tools/terrain_export.sh target/terrain_export` writes a manifest, per-island OBJ meshes, terrain material-weight CSV sidecars with material-region coverage, and `audit.json` without creating a native window; the audit now requires the terrain texture-detail floor and every island's base, transition, highland, and exposed terrain-region coverage above minimum floors.
