...
- `src/scene_setup_runtime/materials.rs` owns startup standard/extended PBR material and procedural texture-handle preparation for player, terrain, clouds, updrafts, water, and detail assets.
...
- `src/content_export/visual.rs` owns the visual-content export facade and stable re-export for generated ground cover, trees, clouds, landmarks, obstruction spires, and biome palettes.
...
- `src/content_export/wind.rs` owns the background-safe wind visual export, using the runtime guide/ribbon transform helpers to write temporal OBJ/NDJSON track artifacts and manifest motion/coherence metrics.
...
- `src/debug_readout_runtime.rs` owns the live HUD/debug readout component, query surface, and diagnostic text formatting; the `--play` run mode skips spawning it.
- `src/debug_visuals.rs` owns F1 debug-visual toggling and Bevy gizmo drawing for player vectors, camera links, visual wind fields, and gameplay lift fields in the default debug sandbox.
...
- `src/eval_app_runtime/metrics.rs` owns startup-versus-runtime app eval frame-time classification, per-sample metric collection, visible generated/authored player transform measurement, and authored player clip-alignment scoring for key-pose readability.
...
- `src/bin/traversal_sim_eval/simulation.rs` owns scripted flight stepping, animation-phase stepping, camera stepping, lift/wind field lookup, power-up collection, and sample capture.
...
- `src/surface_material.rs` owns the shared Bevy `ExtendedMaterial` contract and terrain/water/foam shader uniforms.
- `src/generated_content.rs` owns the generated-content facade and stable mesh/material/texture re-exports.
- `src/generated_content/materials.rs` owns procedural standard and extended PBR material construction for terrain, player, clouds, water, updrafts, and generated detail assets.
- `src/generated_content/surface_textures.rs` owns coherent mipped terrain albedo/normal/ORM generation and crossing-wave water normal generation.
...
- `src/generated_content/detail_meshes.rs` owns the generated detail-mesh facade and stable re-exports for rocks, trees, clouds, route/landing/launch/pond/obstruction-spire landmarks, updraft ribbons, and glider airflow trails.
...
- `src/island_visuals/collision_audit.rs` owns test-only parity checks that generated solid visuals have typed collision proxies, every route island has sampled terrain-rim contour segments, and camera-only blockers are explicitly allowlisted.
...
- `src/movement/orientation.rs` owns desired movement direction, body yaw/roll helpers, facing smoothing, velocity-alignment helpers, and movement-response metrics.
...
- `src/camera/follow.rs` owns camera follow placement, orbit application, movement-stable follow direction, stable movement-facing basis derivation, and follow-direction smoothing.
...
- `src/world.rs` owns the world module facade, shared route/terrain constants including `ISLAND_FOOTPRINT_CONTOUR_SAMPLE_COUNT`, and stable re-exports.
...
- `src/world/island.rs` owns sky-island data, deterministic terrain relief, per-archetype footprint profiles, visual/playable contour sampling, footing/mesh-top helpers, horizontal containment, and per-island LOD/stream activation helpers.
- `src/world/obstruction_spires.rs` owns deterministic per-island route obstruction-spire placement shared by runtime camera/collision blockers, simulation camera-obstruction coverage, and visual-content export.
...
- `src/world_collision_runtime.rs` owns runtime world-asset and terrain-rim collision proxies, player push-out resolution, collision diagnostics, and focused tests for generated prop and rim collisions.
...
- `src/eval/accumulator/movement.rs` owns movement, body-heading, body/travel heading alignment, body-roll, lateral-response, air-brake, pose-readability, and mode-count accumulation.
...
- `src/eval/tests/accumulator_controls.rs` owns accumulator coverage for frame-time, camera/movement drift, air-control response, braking, body heading, body roll, pose-readability, unreadable key-pose rejection, and grounded visual foot-gap gates.
...
- `src/animation.rs` owns primitive character part pose math, body-local pose velocity, pose-readability metrics/scoring, wing visibility/airflow state, and animation phase progression.
...
7. Route ground contact resolves against the deterministic island surface, then `world_collision_runtime::resolve_world_collisions` pushes the player footprint horizontally out of spawned prop proxies and sampled terrain-rim contour segments, clearing inward velocity.
...
9. Character pose phase advances from delta time and speed in both the app and windowless traversal simulation.
10. `animation::body_local_pose_velocity` maps world velocity into Nau's facing frame before pose selection, so lateral turn lean and key-pose metrics follow the character body instead of camera/world axes.
11. `animation::part_pose` maps flight mode, body-local velocity, input, and route height into visible body/glider poses, including grounded walk/run stride, readable launch/fall, dive, air-brake, landing flare/crouch, signed turn lean, and speed-responsive wing flex.
12. Runtime evals sample `animation::pose_readability_metrics` for torso pitch, arm spread, leg tuck, unsigned and signed lateral lean, landing crouch/flare, wing-airflow strength, and key-pose readability; air-control, pose-state, and landing-required scenarios gate those values before counting pose coverage as readable.
13. Runtime wingtip airflow trails reuse the same pure wing-airflow strength helper so visual pressure follows gliding speed without changing gameplay forces.
14. Mouse deltas update `CameraControlState` yaw/pitch when the cursor is locked or right mouse is held.
15. `camera::movement_input_stable_follow_direction` ignores sideways/backward movement input when choosing the camera follow heading, `camera::update_follow_direction_state` smooths that heading independently from mouse orbit, and `camera::step_camera_with_direction` applies yaw/pitch orbit offsets once before smoothing position and rotation.
16. The camera avoids tagged obstruction volumes, including route-integrated generated spires shared with the simulation harness and generated tree/prop blockers in the app route, using shoulder/vertical fallback, a readable boom floor, and transparent-near-prop behavior before floor lifting above the active collision terrain surface.
17. Island terrain is resident inside the active stream window, inactive or non-near chunks show layered distant impostors with measurable mesh and vertex-color complexity, route beacons remain resident for readability, and nonessential island detail, including dense curved-blade ground-cover meshes, is despawned outside the near LOD band.
18. Resident near-LOD tree and pond visuals apply wind-sway transforms from pure environment motion math. Ground cover uses deterministic curved blade clusters, trees use deterministic multi-ring tapered trunk meshes with branch mass/root flares plus overlapping canopy lobes with organic detail cards, and generated detail materials are selected from the same per-island biome family as the terrain, but this motion is visual-only and does not alter route surfaces, camera obstacles, or gameplay forces.
19. HUD text reports frame time, mode, speed, altitude, camera pitch/distance/framing/motion/orbit alignment, obstruction adjustment, mouse yaw/pitch offsets, velocity, power-up visible/collected/active counts, visual asset slots, missing/deferred/queued/loading/loaded/failed load-state buckets, recursive dependency preload buckets, spawned/ready scene-instance buckets, visible authored world-fixture count, declared/ready animation clip counts, linked animation players, ready animation graphs, asset residency classes, visual wind-field count, lift-field count, world-collision proxy/resolution/push metrics, terrain surface vertex/color/material-weight/material-region/texture-detail/relief/cliff-band metrics, distant island impostor mesh/color-band metrics, procedural-vs-primitive island body counts, island body silhouette and mesh min/max complexity, ground-cover patch/blade/vertex metrics, generated detail biome-palette count, active chunk window, near/mid/far LOD island buckets, visible/hidden terrain, impostor, detail counts, environment-motion count/offset, resident island visual count, stream entity churn, route beacons, cooldown, and launch readiness.
20. Debug gizmos draw player vectors, the camera line, visual wind/updraft streams, and gameplay lift-field bounds.
21. The render stack uses Bevy-native atmosphere, dynamic sun/fog/exposure weather, volumetric fog/light, bloom, filmic tonemapping, and one bounded `ExtendedMaterial` surface shader layered onto Bevy's standard PBR input. Terrain combines mipped coherent albedo/normal/ORM maps, authored vertex color, `UV_1` highland/exposed masks, world-space macro/detail variation, and bounded shadow fill. Water and foam use generated crossing-wave normals, mesh tangents, UV/flow/foam masks, oriented front/back normals, basin shimmer, channel/waterfall motion, non-shadow-casting translucency, and explicit unlit foam. The same stack retains per-archetype island contours, ravine/channel incisions, terrace/shelf variation, fine microrelief, biome detail palettes, stratified cliff/underside meshes, layered distant impostors, cinematic wind visuals, cloud banks, and wind-responsive near-LOD environment motion.
22. The default world is a 41-island archipelago with 19 route terrain archetypes, low near-ground cays, broad mid-route anchor islands, high summit islands, varied procedural terrain materials, bounded generated terrain incisions/shelves/microrelief, procedural island bodies, far-LOD layered island impostors, collidable route landmarks/trees/rocks/obstruction spires, 16 sampled terrain-rim collision segments per near-LOD island, fourteen gameplay updrafts paired with sixteen crosswind fields, batched near-LOD ground cover, and a three-gate aerial boost route.
23. The `--export-terrain` CLI path runs without creating a Bevy window and writes per-island terrain/cliff/underside/impostor OBJ meshes, terrain material-weight CSV sidecars, and a manifest of mesh/material/texture-detail/texture-edge/height-band/normal-slope-band/impostor floors for offline inspection; `terrain_export_audit` is a thin directory-backed binary whose manifest, artifact, threshold, and check modules validate that manifest, OBJ topology/color counts, OBJ-derived height-band and normal-slope-band counts, material-weight sidecars, terrain texture-detail and texture-edge floors, terrain/body/impostor silhouette and vertical-mass floors, derived material-region coverage, and minimum base/transition/highland/exposed region distribution agree.
24. The `--export-visual-content` CLI path also runs without creating a Bevy window and writes generated ground-cover/tree/cloud/landmark OBJ meshes plus a manifest of vegetation, cloud, landmark, obstruction-spire, and detail-palette structural metrics; the visual export facade delegates report serialization, export orchestration, mesh metrics, vegetation metrics, cloud artifact writing, and palette summaries to separate modules, and `visual_content_audit` validates artifact presence, OBJ vertex/face counts, blade density and height variance, multi-ring trunk mesh floors, trunk taper, branch reach/count, root-flare count, canopy lobe/detail-card structure, tree height/canopy-radius variation, cloud veil plus lobe/wisp-card/filament-ribbon/depth-span floors, route-cairn/launch-beacon/landing-marker/pond-surface count and mesh/span floors, obstruction-spire vertex/triangle/vertical-span and height/radius/normal-slope band floors, and terrain/detail palette diversity.
25. The `--export-wind-visuals` CLI path runs without creating a Bevy window and writes wind-guide/ribbon temporal motion tracks as OBJ line segments plus NDJSON records over the same short coherence window used by runtime wind metrics; `wind_visual_export_audit` validates artifact counts, expected updraft/crosswind guide/ribbon coverage, non-static motion, bounded off-field/low-alignment samples including an updraft-ribbon family cap, and recomputed `WindField::flow_at` displacement/containment/coherence parity so static or detached wind-current visuals fail before screenshot review.

Eval samples include desired body-yaw/heading error, body/travel heading error, desired-heading velocity alignment, desired-travel heading error, movement input axes, lateral response speed, lateral input activity, pose intent, pose torso pitch, arm spread, leg tuck, unsigned/signed lateral lean, grounded stride foot travel, grounded stride leg opposition, landing crouch, wing-airflow strength, key-pose readability score, key-pose transition grace, visible pose part count, key-pose pose-part rotation/translation deltas, camera distance, camera surface clearance, camera-to-player framing angle, per-frame camera step and rotation deltas, camera orbit alignment, camera follow-direction error, camera view yaw relative to the smoothed follow direction, camera world yaw, obstruction adjustment/hits, camera yaw/pitch offsets, route objective progress, `active_lift_fields`, power-up visibility/collection/effect counters, sky-island count, active chunk count, active island count, near/mid/far LOD island counts, visible/hidden island terrain counts, visible/hidden island impostor counts, minimum island impostor mesh vertex and color-band counts, visible/hidden island detail counts, visible route beacon count, weather cloud count, visual wind field/guide/ribbon counts, per-field wind visual coverage/coherence counts, and guide/ribbon motion/rise/direction-flow metrics, environment-motion visual count and offset, world-collision proxy count/resolved samples/meaningful contact samples/max push, island terrain surface count, terrain mesh vertex floor, terrain vertex-color band floor, terrain material-weight band/channel/region floors, terrain texture-detail floor, terrain relief range, cliff color-band floor, procedural island body count, primitive island body count, island body silhouette complexity, island body mesh vertex floor and max, generated ground-cover density, generated tree/cloud/rock mesh complexity, generated landmark total/per-kind counts and mesh vertex floor, generated cloud-bank depth, generated cloud filament-ribbon detail count, generated detail biome-palette count, resident/catalog/hidden island visual counts, resident visual fraction, directional stream spawn/despawn churn, declared visual asset slot/readiness counts, Bevy scene load-state buckets including deferred admissions, recursive dependency preload counts, spawned/ready scene-instance buckets, visible authored world-fixture count, declared/ready animation clip counts, animation-player/graph readiness counts, asset residency class counts, and entity count. Eval summaries include frame-time avg/p95/p99/max telemetry, average/p95/max body-heading error, aggregate plus right/left/backward-right/backward-left desired-travel heading sample counts plus p95/max error, directional right/left and backward-right/backward-left body/travel heading sample counts plus p95/max error, visible pose part count plus aggregate and right/left readable airborne turn-pose sample counts, readable right/left plus backward-right/backward-left air-brake pose sample counts, global and landing-only temporal stability samples and max pose-part rotation/translation deltas, max yaw-error step, lateral response, separate right/left/rear-right/rear-left air-control response, rear-right/rear-left rearward response, total and planar air-brake recovery, pose-intent counts that only count readable key-pose samples, pose-state grounded idle/walk/run plus launch/fall/glide sample counts in app and simulation summaries, grounded walk/run stride maxima, app-authored idle/walk/run/launch/glide/fall/land clip sample counts, pose-readability maxima including landing flare and signed right/left pose lean, key-pose readability min/max, transition grace, and unreadable-sample count, yaw-oscillation, `lifted_samples`, power-up inventory/collection/effect checks, objective-progress checks, camera-control/framing/orbit/follow-direction/view-yaw/world-yaw/obstruction/jerk checks including minimum obstructed camera distance and repeated obstruction snap count, checkpoint screenshot and marker-metadata artifact paths, sky-island/content-scale/world-collision checks, terrain mesh/color/material-weight/material-region/texture-detail/relief/cliff-band checks, distant-impostor mesh/color-band checks, procedural island body, primitive-body, island silhouette-complexity, and island body mesh-floor checks, ground-cover density checks, generated tree/cloud/rock shape checks, generated landmark total/per-kind counts and mesh checks, cloud-bank depth and filament-ribbon checks, detail biome-palette checks, asset-slot inventory, deferred-load, failed-load and dependency-preload checks, scene-instance readiness, visible authored world-fixture readiness, animation-readiness telemetry, streaming/LOD planning checks, stream-churn checks, resident-pressure checks, weather-cloud checks, environment-motion checks, resident visual/churn checks, visible-detail/beacon checks, and scene entity-count checks. Screenshot evals launched through `tools/eval.sh` add projection-backed checkpoint marker sidecars for known route beacons, route objectives, uncollected aerial power-ups, and terrain/foliage/cloud/distant-island scene samples, a `marker_projection_audit.json` for marker-colored pixels near projected visible markers, a `semantic_scene_audit.json` for material-family, scene-kind, terrain material/biome variant diversity, and variant-aware terrain pixel coverage near projected non-occluded scene samples, and a non-golden `visual_audit.json` for image resolution, exposure, contrast, color variety, edge density, per-frame scene coverage, center-scene detail, scene detail tile frequency, flat low-detail scene-tile dominance, dominant low-detail scene-component dominance, per-frame player visibility, per-frame severe border clipping, non-opaque PNG alpha, large foreign bright-canvas regions, HUD-text dominance, sequence-level route-marker readability/component identity/hue diversity, sequence-level distant horizon/impostor component readability/color-bucket/span identity, sequence-level terrain/material family diversity and terrain material coverage/color/tile spread, sequence-level foliage coverage/tile spread, sequence-level cloud-layer coverage/component/span identity, and sequence-level sky coverage; `terrain_surface` sidecars carry terrain material variant identity for projected diversity gates, screenshot pixels are matched with tolerant per-variant terrain color rules rather than exact per-pixel biome ownership, and `world_collision_contact` applies a close-obstruction semantic profile that keeps close terrain/foliage/distant-island evidence without requiring clouds or broad terrain-variant sequence coverage. `updraft_route` verifies gameplay lift, wind guide/ribbon flow coherence, wind per-field coverage, sustained visual-flow sample windows, and objective progress, `branch_recovery_route` verifies the full branch objective sequence and landing-specific pose temporal quality, `long_glide_visibility` verifies the 41-island archipelago traversal, aerial boost collection, and content scale, `world_collision_contact` verifies generated launch-mesa collision plus nearby tree/prop camera-obstruction readability, `camera_mouse_control` verifies mouse X/Y behavior plus route-spire obstruction readability, `camera_yaw_stability` verifies that small yaw input does not drift after input stops, `camera_strafe_stability` verifies that `A`/`D` movement does not auto-orbit the camera by bounding view-yaw/world-yaw drift while proving right and left strafe response, `camera_turn_stability` verifies rapid airborne turn and air-brake camera stability, `air_control_response` verifies diagonal/lateral/rear-right/rear-left/brake/recovery air control, aggregate plus directional desired-travel heading alignment, non-vacuous directional body/travel heading alignment, readable air-turn/dive/air-brake pose quality with signed right/left lean, right/left turn-pose samples, and right/left/backward-right/backward-left air-brake pose samples, bounded global visible pose temporal deltas, and zero unreadable key-pose samples without movement-driven camera orbit or view-yaw drift, and `pose_state_coverage` verifies grounded idle hold, grounded walk/run stride readability, readable launch/fall/glide, pure-sideways right/left AirTurn and backward-right/backward-left diagonal air-brake sample coverage, matching app-authored idle/walk/run/launch/glide/fall/land clip samples, and zero unreadable key-pose samples on a compact no-mouse route.

Landing pose evals now treat landing anticipation and post-contact recovery as distinct readable key poses. Samples include landing feet-forward distance, asymmetric lead/trail foot split, and landing recovery backbend angle, and summaries gate a recovery-specific maximum backbend so generic torso pitch or an extreme U-shaped recovery cannot satisfy the landing-recovery check.
...
- Visual `WindField` volumes may apply bounded horizontal airborne current through reusable force rules; they must not create vertical lift.
...
- Crosswind and updraft-swirl movement response rules belong in reusable/testable code, not directly in ECS systems.
...
- Visual polish should prefer Bevy-native rendering components and generated assets. A custom material shader is justified only behind Bevy's material extension path with runtime/export parity, bounded material/pipeline counts, focused shader-contract tests, visual/semantic audits, and measured frame-time evidence; new render passes or raw platform code still require a demonstrated limitation.
