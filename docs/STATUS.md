...
2. Add richer screenshot-level terrain-material identity, vegetation/cloud readability, and exact route-marker semantic checks now that the headless export audits gate structural terrain and generated visual substrate quality.
3. Refine distant-impostor screenshot gates from horizon-band image identity into exact world-semantic checks once camera/checkpoint projection metadata is available.
