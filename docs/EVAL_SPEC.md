...
- pose-intent sample counts for readable gliding, launching, falling, diving, air brake, landing anticipation, and landing recovery; app and simulation summaries also count grounded walk/run pose samples, deployed-glider dive samples, right/left air-turn samples, and right/left plus backward-right/backward-left air-brake pose samples, pose-state checks require both right and left air-turn samples, and app evals score visible generated or authored player part transforms when those nodes are available and still require authored clip alignment when authored player geometry hides generated fallback parts
...
- `air_control_response`: current diagonal/lateral/rear-right/rear-left/brake/recovery air-control response test with a bounded transition-grace budget for ambiguous airborne key-pose blends.
