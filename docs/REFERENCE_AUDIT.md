...
5. Continue the internal island mesh generator inspired by `bevy_generative`: NAU now has irregular island masks, rim/cliff/underside geometry, computed normals, tiled terrain UVs, deterministic vertex-color/strata signals, encoded terrain material-weight semantics, derived material-region gates, terrain texture-detail gates, collision surface compatibility, streaming counters, and audited offline OBJ/CSV/manifest export; the next terrain branch should move from material readability toward richer biome identity, vegetation density, and screenshot-level terrain/material semantic checks.
