...
- pose-intent sample counts for readable gliding, launching, falling, diving, air brake, landing anticipation, and landing recovery; app and simulation summaries also count grounded walk/run pose samples, deployed-glider dive samples, right/left air-turn samples, same-sample pure-sideways AirTurn alignment samples, and right/left plus backward-right/backward-left air-brake pose samples, pose-state checks require both right and left air-turn samples, and app evals score visible generated or authored player part transforms when those nodes are available and still require authored clip alignment when authored player geometry hides generated fallback parts
- pose torso pitch, arm spread, leg tuck, lateral lean, signed right/left lateral lean, landing crouch, landing foot-forward tuck, landing foot split, landing flare, wing-airflow maxima, scarf stream/sway/flex maxima, authored glider response/motion maxima, authored glider dive response/motion maxima, authored bank-left/bank-right/fall/dive/air-brake/landing clip sample counts, key-pose readability min/max, unreadable key-pose sample count, and key-pose transition-grace sample count, including bounded app grace for short air-brake, glide, dive, and landing-anticipation release blends; focused pose unit tests also require dive-specific phase-staggered airflow motion across torso, head, arms, and legs while preserving key-pose readability
...
- `pose_state_coverage`: current full-pose chain coverage for walk/run stride, launch/fall/glide, right and left air-turn, air-brake/gliding-dive, landing anticipation/recovery shape, authored-jog/authored-fall, zero-unreadable pose-state coverage, and bounded transition-grace samples.
