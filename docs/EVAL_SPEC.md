...
- camera obstruction avoidance was exercised when a scenario requires it; `camera_mouse_control` specifically exercises the shared route obstruction-spire blockers, and `world_collision_contact` exercises generated launch-mesa tree/prop blockers with minimum readable obstructed camera distance plus zero repeated obstruction snaps so standalone test cuboids cannot be the only obstruction coverage
