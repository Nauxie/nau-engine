...
- Release baseline and play-profile frame-time metrics skip warmup and remain local native-window trend evidence rather than portable cross-machine truth. The development-play gate uses a hidden metric-only native window, runs a separate warmup, and refuses timing evidence from a busy local host before enforcing a maximum 12 ms average, 18 ms p95, 35 ms single frame, at most 24 frames over 16.67 ms, and at most 128 materials. Visible presentation diagnostics remain focus/compositor-sensitive and do not replace focused human release play. `macos-latest` uses calibrated 70/90 ms average/p95 ceilings plus noise-tolerant 500 ms/201-frame overrides and the shared 1.25 same-host debug/release ratio, while the hardware-independent 116-material cap remains the cross-machine regression guard.
