...
- active lift should be readable through paired updraft visuals, breathing lift haze, gusting/advection-driven guide motes/ribbons, crosswind stream-to-stream speed variation, curved tapered crosswind ribbons, layered visual depth, flow-driven scale pulse, and debug bounds before richer particles, cloth/glider motion, vegetation, clouds, or other environment art
...
- `pose_state_coverage` eval tracks grounded walk/run samples, walk/run stride foot travel, walk/run leg opposition, and readable launch, fall, and glide key-pose samples in both the app and windowless sim harnesses; app evals also gate matching authored `jog` and `fall` clip samples
