...
- `Shift` dives.
- Player posing now derives a shared pose intent for grounded idle/stride, launch, fall, glide, explicit airborne turn, dive, air brake, landing anticipation, and post-touchdown landing recovery so generated fallback poses, authored player nodes, authored clip selection, eval labels, and pose-readability metrics stay aligned; the generated and authored player surfaces now use sink/input pressure to scale dive flattening, arms-out spread, head tuck, trailing legs, and deployed-glider wing sweep, exaggerate air-brake arm spread, add directional skid/cupped-limb shape for diagonal air-braking, sink-scaled landing flare/feet-forward leg tuck/crouch, post-touchdown recovery flip/crouch, signed input-responsive airborne turn lean, grounded idle breathing, and subtle glide/dive airflow limb motion, while authored fall clip coverage uses a distinct `fall` clip, authored AirTurn clip selection maps signed lateral input to `bank_left`/`bank_right` clips, and high-sink descents select landing anticipation before the contact frame.
...
- Visual `WindField` volumes now drive both readable airflow visuals/diagnostics and bounded horizontal airborne current: crosswinds push laterally, updraft swirl bends horizontal motion, the near route updraft overlaps an angled crosswind layer, wind guide/ribbon visual motion, stream variation, depth span, scale pulse, coherence, and force-to-flow alignment are checked against shared `WindField::flow_at` vectors, and vertical climb remains separate bounded `LiftField` lift.
...
- The generated island-visual catalog now has a unit audit that classifies every named solid visual by expected proxy kind, requires four terrain-rim proxies per route island, and fails camera blockers without collision unless they are explicitly allowlisted camera-only canopies or cliff-body blockers.
...
- `air_control_response` eval proves diagonal/lateral airborne steering, separate right/left/rear-right/rear-left response latency, rear-right/rear-left rearward response, stronger total and planar backward braking, pure-backward body-heading intent, readable right/left airborne turn-pose coverage, readable deployed-glider dive and directional air-brake pose coverage, visible generated/authored part or authored-clip-aligned pose quality, authored `bank_left`/`bank_right` plus dive/air-brake clip coverage with zero clip mismatches, visible pose part coverage with bounded key-pose rotation/translation deltas, pose torso pitch/arm spread/leg tuck/turn lean/wing airflow readability, visible authored glider response/motion including dive-specific response/motion gates, signed right/left pose lean, key-pose score coverage with zero unreadable key-pose samples, post-brake recovery, desired-heading and aggregate plus right/left/backward-right/backward-left desired-travel alignment, bounded average/p95/max body-heading error, stricter desired-travel and body/travel heading error bounds for omnidirectional air control, non-vacuous right/left and backward-right/backward-left body/travel heading samples with bounded error, bounded same-intent max yaw-error step, zero same-intent yaw oscillation, left/right body-bank response, bounded body-roll step, zero movement-driven camera orbit offset, bounded camera rotation delta, bounded average/p95 camera follow-direction error, and bounded movement-only view-yaw/world-yaw drift.
- `pose_state_coverage` eval proves a compact no-mouse walk/run/launch/fall/glide route emits grounded walk/run samples with readable foot travel and leg opposition, readable launch/fall/glide samples, and zero unreadable key-pose samples in both the app and windowless sim harnesses; the app path also gates matching authored `jog` and `fall` clip coverage.
- `updraft_route` eval proves a scripted route enters a gameplay lift field, sees a paired visible updraft while lift is active, samples soft-edged gust-cell wind flow with nonzero speed/gust variation/range, applies sustained bounded lateral wind response, gates simultaneous layered crosswind-plus-updraft swirl force overlap, gates updraft/crosswind guide and ribbon motion, per-field guide/ribbon coverage, sustained visual-flow sample windows, depth span, scale pulse, and coherence against shared field flow with tightened floors that fail nearly static wind visuals, while focused unit gates prove traveling gust-packet fronts move through the visual streams, and gains altitude only through `LiftField` lift.
- `branch_recovery_route` eval proves a scripted route can target and land on the named `sunlit terrace` branch island after using readable lift and late air-braking while gating readable landing anticipation, authored landing clip coverage, landing flare, feet-forward landing tuck, post-contact landing recovery, landing recovery flip, landing crouch depth, zero unreadable key-pose samples, and landing-only pose temporal stability.
...
- Eval summaries now include the scenario target island, route objective progress, movement/camera/pose metrics including landing feet-forward readability, recovery-flip readability, and deployed-glider dive samples, authored clip match/mismatch and bank-left/bank-right/fall/dive/air-brake/landing coverage metrics, visible authored glider response metrics including dive-specific response/motion, global and landing-only pose temporal metrics, dynamic wind-flow field/speed/variation/range metrics, split wind guide/ribbon direction-flow metrics including updraft swirl displacement, wind visual depth/scale pulse, per-field guide/ribbon/coherence coverage, sustained wind/updraft/crosswind visual-flow sample counts, and short-horizon flow-coherent visual counts/alignment, wind-force field/delta/flow/variation metrics plus sustained meaningful, force-flow-aligned, layered, and crosswind-updraft-overlap sample counts, world-collision and terrain-rim collision metrics, content/asset/streaming metrics, aerial power-up counters, and readable/unreadable/dynamic-readable lift samples so camera/control/content/collision/streaming and wind-current regressions are visible in metrics.
