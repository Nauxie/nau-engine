...
11. `animation::part_pose` maps flight mode, body-local velocity, input, and route height into visible body/glider poses, including readable dive, air-brake, landing crouch, turn lean, and speed-responsive wing flex.
12. Runtime evals sample `animation::pose_readability_metrics` for torso pitch, arm spread, leg tuck, lateral lean, landing crouch, wing-airflow strength, and key-pose readability; air-control and landing-required scenarios gate those values before counting pose coverage as readable.
