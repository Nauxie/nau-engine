...
The environment-motion fields report how many resident near-LOD visuals are wind-responsive and the largest sampled transform offset from their base placement. Dynamic wind-flow fields report how many visual wind volumes contain the sampled player position, the strongest shared flow speed, and the strongest gust variation sampled from the same shared `WindField` catalog used by updraft and crosswind guide visuals. The wind guide visual fields separately count updraft motes/ribbons and crosswind motes/ribbons, then report the largest sampled guide/ribbon offset from its baseline placement, so missing or static airflow cues cannot hide behind generic tree/cloud sway. Summary metrics also track the sampled variation range while lift is active/readable, so a nonzero but static wind-flow value cannot satisfy lift-required scenarios. Together they prove the visual motion layer and readable lift flow are active; they do not evaluate final animation quality.
...
- The simulation-only binary intentionally skips renderer, screenshots, frame-time, asset-server, generated-asset collision contact, and visual content checks; pair it with app/screenshot/export audits before treating a branch as fully verified.
