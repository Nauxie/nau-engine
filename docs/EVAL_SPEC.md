...
This builds and runs the same full-content native scenario through debug and release, verifies identical entity/mesh/triangle evidence, enforces debug and release average/p95 frame-time ceilings, and limits the debug-to-release average frame-time ratio. Local runs default to 24/30 ms average/p95 ceilings. The hosted virtual-M1 CI runner uses calibrated 70/90 ms absolute ceilings while retaining the same 1.25 debug/release ratio, so hardware speed does not hide a development-profile regression or turn a local M4 budget into a false CI failure. Its static companion tests lock both threshold surfaces and the Cargo development optimization contract so Bevy cannot silently return to an unplayable `opt-level = 0` path.
...
- Release baseline and play-profile frame-time metrics skip warmup and remain local native-window trend evidence rather than portable cross-machine truth. The development-play gate is narrower: local runs keep the 24/30 ms playability ceilings, while macOS CI uses calibrated 70/90 ms virtual-runner ceilings and the shared 1.25 same-host debug/release ratio to protect the ordinary `cargo run` path without treating unlike hardware as equivalent.
