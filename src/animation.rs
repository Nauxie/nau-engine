...
pub struct PoseReadabilityMetrics {
    ...
    pub landing_distal_foot_split_m: f32,
...
pub const LANDING_MAX_FOOT_SPLIT_READABILITY_M: f32 = 0.46;
pub const LANDING_MAX_ANTICIPATION_BACKBEND_DEGREES: f32 = 78.0;
pub const LANDING_MAX_RECOVERY_BACKBEND_DEGREES: f32 = 28.0;
pub const LANDING_MAX_TORSO_LOCAL_BEND_DEGREES: f32 = 48.0;
pub const LANDING_MAX_TORSO_LOCAL_OFFSET_M: f32 = 0.24;
pub const LANDING_MAX_TORSO_BACKWARD_BEND_DEGREES: f32 = 8.0;
pub const LANDING_MIN_TORSO_FORWARD_FOLD_DEGREES: f32 = 24.0;
...
pub fn pose_blend_for_intent(intent: PlayerPoseIntent, dt: f32) -> f32 {
    let rate = match intent {
        PlayerPoseIntent::LandingAnticipation => 22.0,
        PlayerPoseIntent::LandingRecovery => 16.0,
...
pub struct PlayerWindShearResponse {
    ...
    pub lateral_shear: f32,
    pub wind_load: f32,
    pub gust_pulse: f32,
}

#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct PlayerWindShearVisualEstimate {
...
pub fn player_wind_shear_response(
    mode: FlightMode,
    velocity: Vec3,
    ...
    let acceleration_pressure = previous_velocity
        .filter(|_| dt > 0.0)
        .map(|previous| {
            ((velocity - previous).length() / dt).min(PLAYER_WIND_SHEAR_MAX_ACCEL_MPS2)
                / PLAYER_WIND_SHEAR_FULL_ACCEL_MPS2
        })
        .unwrap_or(0.0)
        .clamp(0.0, 1.0);
    let gust_pulse = (elapsed_secs * (10.5 + speed_pressure * 5.0)
        + lateral_shear.abs() * 1.7
        + wind_force_delta_mps * 0.22)
        .sin()
        * 0.5
        + 0.5;
    ...
    PlayerWindShearResponse {
        ...
        lateral_shear,
        wind_load: wind_lateral_load.clamp(-1.0, 1.0),
        gust_pulse,
    }
}
...
pub fn player_pose_intent(context: PlayerPoseContext) -> PlayerPoseIntent {
    ...
    if context.mode == FlightMode::Grounded && context.landing_recovery_strength() > 0.04 {
        return PlayerPoseIntent::LandingRecovery;
    }

    let near_landing = context.mode == FlightMode::Airborne
    ...
    match context.mode {
        ...
        FlightMode::Gliding if context.input.dive => PlayerPoseIntent::Diving,
        FlightMode::Gliding if airborne_turn_input(context) => PlayerPoseIntent::AirTurn,
        FlightMode::Gliding => PlayerPoseIntent::Gliding,
        FlightMode::Airborne if context.input.dive => PlayerPoseIntent::Diving,
        FlightMode::Airborne if context.velocity.y > FALL_POSE_MAX_UPWARD_VELOCITY_MPS => {
...
fn landing_anticipation_strength(context: PlayerPoseContext, intent: PlayerPoseIntent) -> f32 {
    ...
    (0.42 + ground_proximity * 0.36 + sink_rate * 0.22).clamp(0.0, 1.0)
}
...
fn landing_flip_strength(context: PlayerPoseContext, intent: PlayerPoseIntent) -> f32 {
    ...
    (proximity * 0.30 + fast_sink * (0.15 + proximity * 0.45)).clamp(0.0, 1.0)
}
...
pub fn key_pose_readability_score(
    ...
    match intent {
        ...
        .min(readable_at_most_score(
            torso_pitch_degrees,
            LANDING_MAX_RECOVERY_BACKBEND_DEGREES,
        ))
        .clamp(0.0, 1.0),
        PlayerPoseIntent::Gliding => {
...
fn torso_signed_backward_bend_degrees(rotation: Quat) -> f32 {
    let local_up = rotation * Vec3::Y;
...
pub fn pose_readability_metrics_from_part_transforms(
    ...
    let (average_leg_lift, average_forward_tuck) = if landing_pose {
        (
            ...
            ((parts.left_leg_translation.z + parts.right_leg_translation.z) * 0.5).max(0.0)
                + average_leg_rotation_radians * 0.40,
        )
    } else {
        (0.0, 0.0)
    };
    ...
    let torso_pitch_degrees = parts
        ...
        .angle_between(Quat::IDENTITY)
        .to_degrees();
    let torso_backward_bend_degrees = torso_signed_backward_bend_degrees(parts.torso_rotation);
    ...
    let mut key_pose_readability_score = key_pose_readability_score(
        context.intent(),
        torso_pitch_degrees,
        arm_spread_degrees,
        leg_tuck_degrees,
    ...
    if context.intent() == PlayerPoseIntent::LandingAnticipation {
        let forward_fold_degrees = (-torso_backward_bend_degrees).max(0.0);
        key_pose_readability_score = key_pose_readability_score
            .min(readable_at_most_score(
                torso_backward_bend_degrees.max(0.0),
                LANDING_MAX_TORSO_BACKWARD_BEND_DEGREES,
            ))
    ...
    PoseReadabilityMetrics {
        ...
        landing_foot_forward_m,
        landing_foot_split_m,
        landing_distal_foot_split_m: 0.0,
    ...
    match part.role {
        CharacterPartRole::Hips => {
            let pitch = match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => {
                    -0.34 - landing_strength * 0.14 - landing_flip * 0.20
                }
                PlayerPoseIntent::LandingRecovery => -0.04 - recovery_strength * 0.10,
            };
            translation.y += match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => -0.010 - landing_flip * 0.010,
                PlayerPoseIntent::LandingRecovery => -0.018 - recovery_strength * 0.025,
                _ => 0.0,
            };
            translation.z += match intent {
                PlayerPoseIntent::Launching => 0.035,
                PlayerPoseIntent::Diving => dive_pressure * 0.020,
                PlayerPoseIntent::LandingAnticipation => 0.020 + landing_flip * 0.020,
                PlayerPoseIntent::LandingRecovery => 0.015 + recovery_strength * 0.020,
                _ => 0.0,
            };
        ...
        CharacterPartRole::Torso => {
            let pitch = match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => {
                    -0.20 - landing_strength * 0.08 - landing_flip * 0.12
                }
                PlayerPoseIntent::LandingRecovery => -0.02 - recovery_strength * 0.08,
            ...
            if intent == PlayerPoseIntent::LandingAnticipation {
                translation.y += 0.030 + landing_strength * 0.020 + landing_flip * 0.018;
                translation.z += 0.030 + landing_strength * 0.030 + landing_flip * 0.035;
            } else if intent == PlayerPoseIntent::LandingRecovery {
                translation.y -= 0.025 + recovery_strength * 0.025;
                translation.z += 0.025 + recovery_strength * 0.035;
            }
            ...
            let landing_counter_roll = match intent {
                ...
                PlayerPoseIntent::LandingRecovery => {
                    -landing_lead_sign * (0.020 + recovery_strength * 0.015)
                }
                _ => 0.0,
            };
            rotation *= Quat::from_rotation_x(pitch)
        ...
        CharacterPartRole::Arm(side) => {
            ...
            let spread = match intent {
                ...
                PlayerPoseIntent::LandingRecovery => 0.60 + recovery_strength * 0.12,
            ...
            let sweep = match intent {
                ...
                PlayerPoseIntent::LandingRecovery => 0.52 + recovery_strength * 0.24,
        ...
        CharacterPartRole::Leg(side) => {
            ...
            let spread = match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => {
                    0.20 + landing_strength * 0.06 + landing_flip * 0.08
                }
                PlayerPoseIntent::LandingRecovery => 0.22 + recovery_strength * 0.06,
            ...
            let trail = match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => {
                    -0.68 - landing_strength * 0.12 - landing_flip * 0.24 - landing_lead * 0.05
                        + landing_trail * 0.04
                }
                PlayerPoseIntent::LandingRecovery => {
                    -0.56 - recovery_strength * 0.28 - landing_lead * 0.06 + landing_trail * 0.05
                }
            ...
            if intent == PlayerPoseIntent::LandingAnticipation {
                translation.z += 0.38
                    + landing_strength * 0.13
                    + landing_flip * 0.24
                    + landing_lead * (0.095 + landing_strength * 0.025 + landing_flip * 0.040)
                    - landing_trail * (0.045 + landing_flip * 0.010);
                translation.y += 0.11
                    + landing_strength * 0.07
                    + landing_flip * 0.08
                    + landing_lead * (0.030 + landing_strength * 0.015)
                    - landing_trail * (0.018 + landing_flip * 0.010);
            } else if intent == PlayerPoseIntent::LandingRecovery {
                translation.z += 0.17
                    + recovery_strength * 0.12
                    + landing_lead * (0.090 + recovery_strength * 0.030)
                    - landing_trail * (0.045 + recovery_strength * 0.012);
                translation.y += 0.06
                    + recovery_strength * 0.06
                    + landing_lead * 0.010
                    + landing_trail * (0.025 + recovery_strength * 0.012);
            }
            if airborne_pose {
        ...
        CharacterPartRole::LowerLeg(side) => {
            ...
            let knee_pitch = match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => {
                    0.54 + landing_strength * 0.16 + landing_flip * 0.18
                }
                PlayerPoseIntent::LandingRecovery => 0.38 + recovery_strength * 0.18,
            };
            ...
            translation.z += match intent {
                ...
                PlayerPoseIntent::LandingAnticipation => 0.026 + landing_flip * 0.010,
                _ => 0.0,
            };
            let lower_leg_roll = match intent {
                PlayerPoseIntent::LandingAnticipation => {
                    0.10 + landing_strength * 0.04 + landing_flip * 0.03
                }
                PlayerPoseIntent::LandingRecovery => 0.08 + recovery_strength * 0.04,
...
mod tests {
    ...
    #[test]
    fn landing_anticipation_pose_blend_exits_head_down_dive_without_snapping() {
        ...
        let dive_hips = part_pose_with_context(&hips, dive_context, 0.0);
        let dive_torso = part_pose_with_context(&torso, dive_context, 0.0);
        let landing_hips = part_pose_with_context(&hips, landing_context, 0.0);
        let landing_torso = part_pose_with_context(&torso, landing_context, 0.0);
        let mut smoothed_hips_rotation = dive_hips.rotation;
        let mut smoothed_torso_rotation = dive_torso.rotation;
        let mut max_step_delta_degrees: f32 = 0.0;
        for _ in 0..8 {
            let previous_hips_rotation = smoothed_hips_rotation;
            let previous_torso_rotation = smoothed_torso_rotation;
            smoothed_hips_rotation =
                smoothed_hips_rotation.slerp(landing_hips.rotation, landing_blend);
            smoothed_torso_rotation =
                smoothed_torso_rotation.slerp(landing_torso.rotation, landing_blend);
            max_step_delta_degrees = max_step_delta_degrees
                .max(
                    smoothed_hips_rotation
                        .angle_between(previous_hips_rotation)
                        .to_degrees(),
                )
                .max(
                    smoothed_torso_rotation
                        .angle_between(previous_torso_rotation)
                        .to_degrees(),
                );
        }
        let transitioned_metrics = pose_readability_metrics_from_part_transforms(
            landing_context,
            PoseReadabilityPartTransforms {
        ...
        assert!(landing_blend > 0.28);
        assert!(landing_blend < 0.34);
        assert!(dive_blend > 0.20);
        assert!(max_step_delta_degrees <= 60.0);
        assert!(
            transitioned_metrics.torso_pitch_degrees <= LANDING_MAX_ANTICIPATION_BACKBEND_DEGREES
        );
        assert!(
            landing_torso
                .rotation
                .angle_between(Quat::IDENTITY)
                .to_degrees()
                <= LANDING_MAX_TORSO_LOCAL_BEND_DEGREES
        );
        assert!(
            landing_torso.translation.distance(torso.base_translation)
                <= LANDING_MAX_TORSO_LOCAL_OFFSET_M
        );
    }
    ...
    #[test]
    fn player_wind_shear_response_tracks_speed_dive_and_grounding() {
        let fast_dive = player_wind_shear_response(
            FlightMode::Gliding,
        ...
        let slow_ground = player_wind_shear_response(
            FlightMode::Grounded,
    ...
    #[test]
    fn player_wind_shear_response_tracks_acceleration_and_lateral_load() {
    ...
    #[test]
    fn gliding_near_ground_keeps_glider_pose_instead_of_landing_anticipation() {
        let neutral_glide = PlayerPoseContext::new(
            FlightMode::Gliding,
            Vec3::new(0.0, -18.0, -28.0),
            FlightInput::default(),
            4.0,
        );
        let diving_glide = PlayerPoseContext::new(
            FlightMode::Gliding,
            Vec3::new(0.0, -18.0, -28.0),
            FlightInput {
                dive: true,
        ...
        assert_eq!(player_pose_intent(neutral_glide), PlayerPoseIntent::Gliding);
        assert_eq!(player_pose_intent(diving_glide), PlayerPoseIntent::Diving);

        let resolved =
            resolve_pose_intent(PlayerPoseIntent::Diving, 0.12, diving_glide, 1.0 / 60.0);

        assert_eq!(resolved.intent, PlayerPoseIntent::Diving);
    }
    ...
    fn fast_sink_landing_anticipation_flips_more_than_slow_approach() {
        ...
        assert!(
            fast_metrics.torso_backward_bend_degrees
                < slow_metrics.torso_backward_bend_degrees - 8.0
        );
        assert!(slow_metrics.torso_backward_bend_degrees <= 0.0);
        assert!(fast_metrics.torso_backward_bend_degrees <= 0.0);
        assert!(fast_metrics.leg_tuck_degrees > slow_metrics.leg_tuck_degrees + 4.0);
        assert!(fast_metrics.landing_crouch_m > slow_metrics.landing_crouch_m + 0.005);
        assert!(fast_metrics.landing_foot_forward_m > slow_metrics.landing_foot_forward_m + 0.02);
        assert!(fast_metrics.landing_foot_split_m >= LANDING_MIN_FOOT_SPLIT_READABILITY_M);
        assert!(fast_metrics.landing_foot_split_m <= LANDING_MAX_FOOT_SPLIT_READABILITY_M);
        assert!(
            fast_head
                .rotation
    ...
    fn landing_anticipation_readability_requires_torso_flare() {
        ...
        let overbent_score = key_pose_readability_score(
            PlayerPoseIntent::LandingAnticipation,
            LANDING_MAX_ANTICIPATION_BACKBEND_DEGREES + 12.0,
            0.0,
            90.0,
            0.16,
            0.40,
        ...
        assert!(overbent_score < MIN_KEY_POSE_READABILITY_SCORE);
        assert!(readable_score >= MIN_KEY_POSE_READABILITY_SCORE);
    }
    ...
    fn landing_recovery_pose_absorbs_impact_after_touchdown() {
        ...
        assert!(recovery_torso.translation.y < stride_torso.translation.y - 0.035);
        ...
        assert!(
            recovery_metrics.landing_recovery_flip_degrees <= LANDING_MAX_RECOVERY_BACKBEND_DEGREES
        );
    ...
    #[test]
    fn landing_recovery_readability_requires_absorption_without_extreme_backbend() {
        let extreme_backbend_score = key_pose_readability_score(
            PlayerPoseIntent::LandingRecovery,
            LANDING_MAX_RECOVERY_BACKBEND_DEGREES + 18.0,
            0.0,
            48.0,
            0.12,
            0.0,
        ...
        let weak_split_score = key_pose_readability_score(
            PlayerPoseIntent::LandingRecovery,
            LANDING_MAX_RECOVERY_BACKBEND_DEGREES - 4.0,
            0.0,
            48.0,
            0.12,
            0.0,
        ...
        assert!(extreme_backbend_score < MIN_KEY_POSE_READABILITY_SCORE);
        assert!(weak_split_score < MIN_KEY_POSE_READABILITY_SCORE);
        assert!(readable_score >= MIN_KEY_POSE_READABILITY_SCORE);
    }
    ...
    fn pose_readability_metrics_distinguish_dive_air_brake_and_landing() {
        ...
        assert!(landing.landing_foot_split_m <= LANDING_MAX_FOOT_SPLIT_READABILITY_M);
        assert!(recovery.landing_crouch_m > 0.055);
        assert!(recovery.landing_foot_split_m >= LANDING_MIN_FOOT_SPLIT_READABILITY_M);
        assert!(recovery.landing_foot_split_m <= LANDING_MAX_FOOT_SPLIT_READABILITY_M);
