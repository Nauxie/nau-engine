...
2. Add screenshot-level vegetation/cloud readability and exact route-marker semantic checks now that the headless export audits gate structural terrain and generated visual substrate quality.
3. Refine screenshot terrain/material and distant-impostor gates from image-family identity into exact world-semantic checks once camera/checkpoint projection metadata is available.
