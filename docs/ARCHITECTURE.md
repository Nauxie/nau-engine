...
- The player and glider slots currently spawn Bevy `SceneRoot` children if their files exist. The primitive body/wings remain hidden only after those scene instances report `SceneInstanceReady`; failed or missing files leave generated placeholders visible. The player slot also looks for the declared named animation clips, attaches an `AnimationGraph`/`AnimationTransitions` pair to the nested `AnimationPlayer` when all clips are present, and starts the first clip as idle.
