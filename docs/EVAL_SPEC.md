...
The export writes `manifest.json` and `player_pose_sheet.svg`. The sheet is generated from `assets/models/player/player.gltf` and the same runtime pose math used by `asset_fixture_audit`, then lays out front, side, and top silhouettes for grounded idle, launch takeout, belly-down fall, glide, head-down dive, air-brake, landing anticipation, and landing recovery. Dashed overlay lines identify sampled mesh-surface connector distances, so reviewers can see where shoulders, elbows, wrists, hips, knees, or ankles are nearing the contact threshold. This is a review artifact for catching fixture wiring, proportion, and silhouette issues that numeric gates can miss; it is not a golden-image test.
