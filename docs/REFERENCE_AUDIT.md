...
5. Continue the internal island mesh generator inspired by `bevy_generative`: NAU now has irregular island masks, rim/cliff/underside geometry, computed normals, deterministic vertex-color/strata signals, encoded terrain material-weight semantics, derived material-region gates, collision surface compatibility, streaming counters, and audited offline OBJ/CSV/manifest export; the next terrain branch should move from structural material identity toward sharper PBR material treatment and screenshot-level granularity checks.
