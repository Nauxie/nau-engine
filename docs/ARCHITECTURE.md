...
- Expected drop-in paths are `assets/models/player/player.glb`, `assets/models/player/glider.gltf`, `assets/models/world/island_terrain.glb`, `assets/models/world/foliage.glb`, `assets/models/world/rocks.glb`, `assets/models/world/water.glb`, `assets/models/world/route_markers.glb`, `assets/models/world/weather_layers.glb`, and `assets/models/world/island_impostors.glb`.
- The glider slot currently has a self-authored fixture at `assets/models/player/glider.gltf` so Bevy `SceneRoot` load, spawn, and ready lifecycle stays exercised in every eval run. The player and glider slots spawn `SceneRoot` children if their files exist. The primitive body/wings remain hidden only after those scene instances report `SceneInstanceReady`; failed or missing files leave generated placeholders visible. The player slot also looks for the declared named animation clips, attaches an `AnimationGraph`/`AnimationTransitions` pair to the nested `AnimationPlayer` when all clips are present, and starts the first clip as idle.
