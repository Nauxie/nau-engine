...
- pose torso pitch, arm spread, leg tuck, lateral lean, signed right/left lateral lean, landing crouch, landing foot-forward tuck, landing flare, wing-airflow maxima, scarf stream/sway/flex maxima, authored glider response/motion maxima, authored glider dive response/motion maxima, authored bank-left/bank-right/fall/dive/air-brake/landing clip sample counts, key-pose readability min/max, unreadable key-pose sample count, and key-pose transition-grace sample count, including bounded app grace for short air-brake, glide, dive, and landing-anticipation release blends; focused pose unit tests also require dive-specific phase-staggered airflow motion across torso, head, arms, and legs while preserving key-pose readability
