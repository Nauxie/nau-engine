...
fn observe_pose_readability(accumulator: &mut EvalAccumulator, sample: &EvalSample) {
    ...
    if sample.pose_intent_label == "launching" {
        accumulator.authored_glider_launch_samples += 1;
        accumulator.max_authored_glider_launch_response_degrees = accumulator
            .max_authored_glider_launch_response_degrees
            .max(sample.authored_glider_response_degrees);
        accumulator.max_authored_glider_launch_motion_m = accumulator
            .max_authored_glider_launch_motion_m
            .max(sample.authored_glider_motion_m);
    }
