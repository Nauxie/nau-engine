...
12. `camera::update_follow_direction_state` keeps smoothed horizontal travel direction independent from mouse orbit, using player velocity with player forward as the low-speed fallback, and `camera::step_camera_with_direction` applies yaw/pitch orbit offsets once before smoothing position and rotation.
...
- Camera follow direction should use horizontal travel direction, not full 3D velocity, and should fall back to player forward only when horizontal speed is too low to be useful.
