...
`./tools/player_pose_preview.sh target/player_pose_preview` runs without opening the native window. It writes `manifest.json`, `player_pose_sheet.svg`, `player_transition_pose_sheet.svg`, and `glider_pose_sheet.svg`, human-review pose sheets generated from the current player/glider glTF plus the same runtime pose math used by the fixture audit. The player sheet shows front, side, and top silhouettes for grounded idle, launch takeout, belly-down fall, glide, head-down dive, air-brake, landing anticipation, and landing recovery, with dashed surface-contact overlays for visible connector gaps. The transition sheet shows the 50% blend frame for every high-risk traversal pose transition so limb disconnects or clipping that only happen between settled poses are reviewable. The glider sheet uses shared per-view scale across stowed, takeout, glide, dive, and air-brake rows so compact-to-open deployment regressions are visible alongside numeric gates.
