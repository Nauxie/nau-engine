...
- pose-state coverage scenarios record grounded idle/walk/run samples, grounded walk/run stride foot travel and leg opposition, readable launch/fall/glide/air-turn/air-brake/gliding-dive samples with dive-only torso/arm/leg silhouette floors, right and left air-turn pose samples, same-route pure-sideways right/left AirTurn samples, backward-right/backward-left diagonal air-brake samples, landing anticipation/recovery samples, landing crouch, feet-forward flare, asymmetric foot-split floor, recovery flip, scarf stream/sway/flex floors, and app-authored `idle`/`walk`/`run`/`launch`/`glide`/`bank_left`/`bank_right`/`fall`/`land` clip samples while rejecting unreadable key-pose samples
...
- `pose_state_coverage`: current full-pose chain coverage for idle hold, walk/run stride, launch/fall/glide, right and left air-turn, pure-sideways right/left AirTurn, backward-right/backward-left diagonal air-brake, air-brake/gliding-dive, landing anticipation/recovery shape, app-authored idle/walk/run/launch/glide/bank-left/bank-right/fall/land, zero-unreadable pose-state coverage, and bounded transition-grace samples.
