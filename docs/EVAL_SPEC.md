...
Run the app-only terrain-body collision contact route:
...
Run the deterministic 41-island art-direction inventory gate:

```sh
./tools/island_art_direction_gate.sh target/eval/island_art_direction
```

Run the complete 41-island × 3-view hero gallery:
...
Run the app directly without screenshot capture. This skips PNG capture but still creates the native Bevy window:
...
This runs the focused world and generated-content tests, terrain/layout export audit, visual-content export audit, 41-island art-direction audit, simulation-only long-glide plus great-plateau routes, the native movement-plus-mouse camera regression, the app-only great-plateau vista audit, the dedicated `island_surface_review` screenshot/semantic audit, and the full 123-capture island hero gallery. It is the required iteration gate for island layout, scale, terrain, palettes, vegetation, flora, ruins, geological formations, waterscapes, hero landmarks, traversal content, and launch-area camera changes; release app profiling and human play remain required before accepting rendering-density or camera-feel changes.
...
This combines deterministic 30/60/120/144 Hz and 50/100 ms hitch contracts with native app scenarios for mouse response, obstruction transitions, reset handoff, world/terrain collision, and floor-boundary traversal. It requires exact sample coverage, bounded and attributed camera/player corrections, a numeric nonnegative `camera_obstruction_vertical_correction_step_m` no greater than `0.241 m` on every app frame, zero unclassified corrections, and a fault-injection proof that a one-frame snap is rejected.
...
This builds and runs the same full-content native scenario through debug and release, verifies identical entity/mesh/triangle evidence, enforces average, p95, single-frame maximum, over-budget-frame, and material-count ceilings for both profiles, and limits the debug-to-release average frame-time ratio. Local measured runs require the existing host preflight to pass and execute one unmeasured warmup run per profile before collecting evidence. The default metric-only window is hidden so foreground focus, AppKit window transitions, and compositor cadence cannot masquerade as engine-work regressions; set `NAU_DEV_PLAY_PERF_VISIBLE_WINDOW=1` only for presentation diagnostics, and use a focused release play profile plus human play for final feel acceptance. Visible play-profile schema v5 reads the native current monitor through winit, records refresh-rate transitions, evaluates pacing against the highest observed refresh rate, and fails when refresh cadence changes or cannot be observed; profiling automation never requests foreground focus. The gate defaults to a 12 ms maximum average, 18 ms p95, 35 ms maximum frame, at most 24 frames over 16.67 ms, and at most 128 materials. `macos-latest` retains calibrated 70/90 ms average/p95 ceilings, permits a 500 ms maximum frame and all 201 sampled frames over 16.67 ms to absorb virtual-runner scheduling noise, and retains the 1.25 same-host debug/release ratio. CI still caps materials at 116: that structural, hardware-independent limit is the cross-machine protection against material proliferation even when timing budgets need runner-specific calibration. Pull requests also run release `baseline_route`, `long_glide_visibility`, and `camera_mouse_control` against both the base SHA and candidate on the same runner. The release traversal scenarios retain bounded authored-content ceilings of 260 visible details, 430 resident visuals, and 5,500 total entities, while the same-host comparator uses its strict default 10% plus five-count allowance for structural growth. Because the 200-frame camera route is more exposed to hosted-runner stalls, only its average comparison permits 15% relative variance; p95 and every longer-route average remain at 10%. Fixed `>33`/`>50`/`>100 ms` frame buckets are hard relative hitch gates only when the base bucket covers at most 5% of runtime samples. Once a bucket represents ordinary virtual-runner cadence rather than a tail, it remains visible as advisory evidence and p95 is the hard sustained-tail signal. Severe `>100 ms` events therefore remain protected on current runners, and all structural counts always retain the strict 10% plus five-count policy. The preceding development-play gate owns the candidate's absolute timing budgets; the comparison step uses broad 500 ms collection ceilings so a noisy base run cannot abort before the candidate is measured. Collection failures are captured instead of short-circuiting the step, ensuring the comparator always emits its diagnostics before CI propagates any collection or comparison failure. If the first relative comparison fails after both revisions collect successfully, CI reuses the built binaries and repeats all three scenarios in reverse candidate/base order; the PR fails only when that second comparison reproduces a regression or the retry cannot collect valid evidence. Executable comparator fixtures prove that bulk cadence variance passes while true severe-hitch and structural regressions still fail; static companion tests lock the threshold surfaces, retry ordering, relative-comparison wiring, and Cargo development optimization contract so Bevy cannot silently return to an unplayable `opt-level = 0` path.
...
The export writes `manifest.json`, SVG sheets, Chrome-rendered PNG screenshots, and `preview_audit.json` for `player_pose_sheet`, `player_anatomy_review_sheet`, `player_rig_stress_review_sheet`, `player_motion_integrity_review_sheet`, `player_transition_pose_sheet`, `glider_pose_sheet`, and `player_glider_attachment_sheet`. The player sheet is generated from `assets/models/player/player.gltf` and the same runtime pose math used by `asset_fixture_audit`, then lays out front, rear, side, and top silhouettes for grounded idle, grounded walk, grounded run, launch takeout, belly-down fall, glide, streamlined dive, air-brake, landing anticipation, and landing recovery. The anatomy review sheet gives larger panels for full-body proportions, fall/dive silhouettes, shoulder/hip connector closeups, and hand/boot closeups so human review does not depend on tiny full-body thumbnails. The fixture audit also records and gates a launch overhead-arm score so the launch takeout row must keep both arms raised toward deployment instead of passing with a low sideways silhouette. The transition sheet renders the 20%, 40%, 60%, and 80% blend frames for each high-risk traversal pose transition, so fixture gaps and clipping that only appear between settled poses are visible during review. Dashed overlay lines identify sampled mesh-surface connector distances, and translucent rectangles identify near-threshold connector or non-adjacent mesh intersections, so reviewers can see where shoulders, elbows, wrists, hips, knees, or ankles are nearing gap or clipping thresholds. The rendered PNG audit validates SVG/PNG dimensions, rejects foreign bright canvases, requires the dark preview background to dominate, and verifies colored pose/glider detail is present so corrupted screenshots cannot pass just because a file exists. The underlying fixture audit also reports projected visual contact gaps, proximal shoulder/hip mesh overlap, and rest-pose non-adjacent plus shoulder worst-overlap pairs separately so pure AABB intersection cannot hide an actual visible disconnect or deep body connector intersection. The glider sheet is generated from `assets/models/player/glider.gltf` and uses shared per-view scale across stowed, partial takeout, launch takeout, glide, dive-release, and air-brake rows so deployment silhouette changes are reviewable. The attachment sheet renders player and deployed glider silhouettes together across takeout, glide, dive-release, and air-brake rows; hand-to-grip distance overlays are shown and gated only for settled takeout/glide/brake grip poses, while released/folding glider poses remain visible without false grip-error overlays. The manifest and fixture audit gate settled takeout/glide/brake hand-to-grip surface distance, projected gap, sample count, and breach count. The manifest and fixture audit also gate the current limb-anatomy detail layer, including bicep, tricep, upper-arm seam/inner-tendon, shoulder/axilla blend, shoulder/elbow/hip/knee motion cowls, forearm tendon/radius/ulna ridges, finger web, hip/inguinal blend, thigh quad/hamstring panels, calf, shin, lower-leg tendon, instep, boot side/arch, heel tendon, and lace/strap nodes. The manifest and fixture audit also gate high-density organic major-limb volumes for the upper-arm, forearm, thigh, lower-leg, and boot meshes plus reusable hand/finger/boot extremity volumes so the fixture cannot regress to low-density rod, box, or marker-like silhouettes while the connector gates still pass. The manifest and fixture audit also gate human-topology proportions, including shoulder width, hip width, shoulder-to-hip ratio, arm/leg segment ratios, bilateral symmetry, head centering, and neck-to-shoulder rise. These are review artifacts for catching fixture wiring, proportion, deployment, attachment, anatomy, and silhouette issues that numeric gates can miss; they are not golden-image tests.

Export wind-current visuals for offline coherence inspection without creating a native window:

```sh
./tools/wind_visual_export.sh target/wind_visual_export
```

The export writes `manifest.json`, `wind_tracks/wind_visual_tracks.obj`, `wind_tracks/wind_visual_tracks.ndjson`, and `audit.json`. The OBJ is a line-segment artifact for visual inspection; the NDJSON records every sampled guide/ribbon center or mesh sample across adjacent frame windows through the late loop cycle, including whether each endpoint is quality-visible rather than inside the loop-recycle fade. The audit validates schema, artifact presence, OBJ vertex/segment counts, NDJSON track count, full updraft/crosswind field coverage, expected guide/ribbon counts, non-static motion ratio, bounded quality-visible speed/acceleration, continuity gaps, bounded off-field and low-alignment ratios, and recomputed displacement/field-containment/flow-alignment coherence against public `WindField::flow_at` math. This is the background-safe gate for wind visuals being present, moving smoothly while visible, and broadly attached to the same gameplay field model before screenshot review.
...
- summary written after 440 frames
...
- target landing gates also require authored glider response/motion evidence so the route cannot pass as launch-only ballistic travel
...
`terrain_body_collision_contact` is the app-only visible island body collision regression test:

- eval-specific spawn in a clean launch-island east-cliff lane, offset from generated rocks/trees/ridges so the route exercises the cliff body panel rather than decorative asset blockers
- no launch, glide, or dive input
- scripted grounded rightward taxi into the launch-mesa cliff side
- the player must stay grounded
- at least 10 sampled frames must resolve terrain-body collision, with at least 0.04 m max terrain-body push and zero terrain-rim resolved samples, so visible island body blockers cannot pass through aggregate proxy counts or terrain-rim contact

`updraft_route` is the first traversal power-up eval:

- fixed spawn on the launch island
- scripted launch, glide, and steering into the gameplay lift field
- the route must register active lift samples while gliding through the lift volume
...
- the route must sample bounded horizontal wind response from the visual `WindField` while vertical climb remains `LiftField`-capped and uses paired visual updraft flow for thermal-core/gust/edge lift modulation
- neutral gliding frames inside gust-varied shared `WindField` flow must produce readable body/glider reaction coupled to total applied wind-current delta, measured by normalized lateral load, pose lean, and authored glider response gates
...
- route fly-through objectives require gliding inside the corresponding updraft volume, so early horizontal proximity cannot complete a lift objective before the route actually enters usable lift
...
- final target distance, grounded target landing samples, and glider-response evidence must pass against the named branch island
...
- bounded semantic samples identify flora clusters, ruin complexes, rock formations, water details, lakes, channels, and waterfalls near each checkpoint; flora samples target deterministic points on generated mesh geometry rather than arbitrary cluster centers
...
`island_hero_gallery` is the app-only route-wide art-direction eval:

- the review catalog materializes exactly three deterministic views for each of the 41 islands: near, mid, and traversal
- the run writes exactly 123 screenshots and 123 marker sidecars plus `island_review_manifest.json`
- near views require target-scoped terrain, hero, flora, formation, ruin, and water evidence as applicable to the island profile
- mid and traversal views require the target island's readable silhouette and identity without pretending unrelated route content satisfies coverage
- hero evidence uses four deterministic points selected from actual generated landmark triangles; a coarse camera bound cannot be the only proof that the landmark is visible
- semantic island occlusion rejects points outside the authored visual silhouette instead of treating every island as a full ellipse
- route-edge waterfall geometry and review framing share the same placement contract, and non-plateau waterfall-garden near views keep the actual ribbon and mist inside a 0.70 normalized safe frame
- visual, marker-projection, and semantic-scene audits must pass for every checkpoint, and the aggregate gate rejects missing, duplicate, mislabeled, or off-target captures
- automated success is an evidence floor; direct review of all affected captures remains required after art, lighting, framing, LOD, streaming, or collision changes
...
- at least one frame must simultaneously report `camera_correction_source == "input"` and `camera_obstruction_hits > 0`, proving the input-plus-obstruction guard is exercised
- collision-only vertical obstruction correction is measured separately from intentional orbit motion and must stay at or below `0.241 m` per 60 Hz frame
...
- lateral and backward-diagonal body/travel heading error must collect finite right, left, backward-right, and backward-left samples and stay bounded after each direction-specific response window and current-frame lateral response threshold so Nau cannot visibly fly sideways while presenting the wrong body direction or carry stale forward/side drift through a reversal; the sample field is finite only for airborne/gliding lateral-input samples at or above the 6.0 m/s planar speed floor
...
- signed pose lean must agree with input direction, so right input must produce readable right lean, left input must produce readable left lean, and backward-right/backward-left air-brake input must produce matching signed air-brake lean instead of one good unsigned lean sample satisfying both directions
- visible generated/authored pose parts must produce at least one finite temporal sample during key poses, and key-pose part rotation/translation deltas must stay bounded so readable pose coverage cannot hide a one-frame limb snap; focused pose tests additionally cover pressure-scaled dive flattening, leg trail, and rearward air-brake torso/limb/glider cup so shallow/high-sink dives and backward braking do not collapse to fixed key poses; during smooth authored pose transitions, readability may score against the immediately previous key-pose shape, and a brief in-between key-pose sample window of at most five frames may be accepted only when it is already at least 0.65 readable and pose-part deltas stay within 60 degrees/0.15 m
- current gates require lateral response within 0.20 seconds after reaching at least 6 m/s, at least 26 m/s directional right/left lateral response, at least 10 m/s rear-right and rear-left lateral response, at least 10 m/s rear-right and rear-left rearward response, at least 13 m/s total and 14 m/s planar air-brake speed drop, at least 14 m/s of post-brake forward recovery, at least 8 degrees of left and right body-bank response, at least 8 degrees of signed right and left pose lean plus signed backward-right/backward-left air-brake pose lean, at least four readable `air_turn` pose samples with at least four right and four left `air_turn` samples, at least four same-sample pure-sideways readable `air_turn` body/travel and desired-travel alignment samples per side, at least four right, left, backward-right, and backward-left readable `air_brake` samples, at least one visible pose temporal sample, pose-part rotation/translation deltas at or under 120 degrees/0.55 m, sampled body-roll steps at or under 12 degrees, camera view-yaw and world-yaw drift at or under 2 degrees, p95 body-heading error at or under 15 degrees, at least eight desired-travel heading samples with p95/max error at or under 8/26 degrees, at least four right, left, backward-right, and backward-left desired-travel samples, at least four right and left lateral body/travel heading samples plus at least four backward-right and backward-left diagonal body/travel heading samples, pure-sideways `air_turn` body/travel p95/max error at or under the lateral 8/18 degree bounds, pure-sideways `air_turn` desired-travel p95/max error at or under the 8/26 degree bounds, lateral body/travel heading p95/max error at or under 8/18 degrees, backward-diagonal body/travel heading p95/max error at or under 8/12 degrees, max body-heading error at or under 24 degrees, max same-intent yaw-step error at or under 28 degrees, and zero same-intent yaw oscillations; focused movement unit coverage also requires lateral reversal to build readable opposite-side travel while shedding stale forward drift
...
- `island_review_manifest.json`: exact island/view catalog for `island_hero_gallery`, including 41 islands and 123 expected captures.
- `marker_projection_audit.json`: projected visible-marker pixel audit for screenshot evals that emit marker sidecars.
- `semantic_scene_audit.json`: projected terrain/foliage/cloud/distant-island material-family and expected scene-kind hit, conditional wind visual pixel-hit checks, terrain material/biome variant diversity, visible terrain material-variant hit coverage, and aggregate pixel-coverage audit for screenshot evals that emit marker sidecars; `world_collision_contact` uses a close-obstruction profile that omits cloud and broad terrain-variant sequence gates while retaining terrain/foliage/distant-island close-frame material/kind coverage.
...
- `asset_fixture_audit.json`: structural glTF fixture audit run through `tools/eval.sh` unless `NAU_EVAL_ASSET_AUDIT=0` is set, including player mesh-projected silhouette gates, organic major-limb volume gates, extremity hand/finger/boot volume gates, and rest-pose worst-overlap reports for the current pose fixture.
- `player_pose_sheet.svg` / `player_pose_sheet.png`: windowless player fixture pose sheet emitted by `tools/player_pose_preview.sh` for human review, including mesh-surface contact overlays where connector surfaces visibly separate and translucent markers where tracked mesh pairs approach overlap thresholds; its manifest also records the mesh-projected silhouette audit used to gate fall/glide/dive outlines.
- `player_anatomy_review_sheet.svg` / `player_anatomy_review_sheet.png`: larger human-review panels for full-body proportions, shoulder/hip connectors, hands, boots, fall footprint, and dive stack.
- `player_transition_pose_sheet.svg` / `player_transition_pose_sheet.png`: windowless player fixture transition sheet emitted by `tools/player_pose_preview.sh`, showing high-risk 20%, 40%, 60%, and 80% traversal blend frames with the same mesh-surface contact and overlap overlays.
- `glider_pose_sheet.svg` / `glider_pose_sheet.png`: windowless glider deployment pose sheet emitted by `tools/player_pose_preview.sh`, using shared per-view scale so stowed, takeout, glide, dive-release, and air-brake silhouettes can be compared visually.
- `player_glider_attachment_sheet.svg` / `player_glider_attachment_sheet.png`: windowless combined player/glider pose sheet emitted by `tools/player_pose_preview.sh`, showing takeout, glide, dive-release, and air-brake attachment rows; hand-to-grip overlays are reserved for grip-required poses.
...
`NAU_EVAL_SIM_ONLY=1 ./tools/eval.sh <scenario> <dir>` runs the `traversal_sim_eval` binary instead of the app. It writes the same `summary.json` and `samples.ndjson` artifact names with schema `nau_traversal_sim_eval.v2`, reports `mode: "simulation_only"` and `native_window_created: false`, and covers scripted input, flight integration, route objectives, lift fields, bounded horizontal wind response, aerial power-ups, LOD route math, camera follow, shared route-spire obstruction avoidance, and movement/camera checks without creating a Bevy app or native window. The v2 summary adds direction-split desired-travel, AirTurn pose coverage fields, same-sample pure-sideways AirTurn alignment fields, direction-split AirBrake pose coverage fields, explicit grounded-idle, grounded-walk, grounded-run, launching, falling, upward-falling rejection, gliding, and dive-without-input rejection fields, plus deployed-glider response and dive-only torso pitch, arm spread, and leg-tuck maxima. The `air_control_response` sim gates right/left lateral response and latency, backward-right/backward-left diagonal response and latency, aggregate plus right/left/backward-right/backward-left desired-travel heading samples/error, explicit right/left `air_turn` pose samples, same-sample pure-sideways readable `air_turn` body/travel plus desired-travel alignment samples/error, explicit right/left plus backward-right/backward-left `air_brake` pose samples, deployed-glider dive pose torso/arm/leg-tuck floors, body-heading error/oscillation, directional body/travel heading sample coverage and alignment, body-roll/bank response, air-brake speed drop, post-brake recovery, bounded key-pose transition-grace samples, and movement-only camera yaw/rotation drift. The `camera_mouse_control` sim gates yaw/pitch input and nonzero camera-obstruction adjustment from the same route-integrated spires used by the app. The `pose_state_coverage` app and sim gates grounded idle/walk/run samples, walk/run stride foot travel, walk/run leg opposition, readable launch/fall/glide/air-turn/air-brake/gliding-dive samples with dive-only torso/arms/leg-tuck floors, right and left air-turn pose samples, settled pure-sideways right/left AirTurn samples with body/travel and desired-travel p95/max heading-error gates, backward-right/backward-left diagonal air-brake samples, landing anticipation/recovery samples, zero gliding landing-anticipation samples, landing crouch, feet-forward tuck, signed landing forward-fold and backward-bend gates, app local torso-bend/torso-offset gates, zero upward-velocity fall poses, zero dive poses without dive input, zero unreadable key poses, and bounded key-pose transition-grace samples on a compact no-mouse route; the app path also gates matching authored `idle`, `walk`, `run`, `launch`, `glide`, `bank_left`, `bank_right`, `fall`, `dive`, `air_brake`, and `land` clip samples plus launch-specific authored glider takeout sample/rotation/motion response. Target-landing routes gate route-level deployed-glider response in sim and authored glider response/motion in app, so successful landing routes must show glider use in addition to final position. Wind-current scenarios gate active wind-force samples, wind-force delta, source flow speed/variation, and crosswind-specific delta where relevant, so background loops can catch the core flight-control regressions before screenshot/app evals. `./tools/eval_sim_suite.sh <dir>` runs all simulation-supported scripted scenarios through this path, disables per-scenario asset audits, runs `asset_fixture_audit` once, includes pose-state, direction-split air-turn/air-brake, pure-sideways AirTurn alignment, backward-diagonal brake counters, and key-pose transition-grace counters in its aggregate summary, and emits `<dir>/summary.json` with schema `nau_sim_suite.v1` so long background loops can prove those routes stayed windowless. App-only routes such as `world_collision_contact`, `terrain_rim_collision_contact`, and `terrain_body_collision_contact` depend on runtime-spawned Bevy collision proxies and should be run through the default app eval path. Simulation-only evals deliberately do not produce screenshots, marker sidecars, render/content image audits, frame-time telemetry, asset-server lifecycle checks, or asset, terrain-rim, and terrain-body collision contact checks.
`tools/eval.sh` checks that declared PNG artifacts exist, are large enough, that declared marker sidecars exist and pass their projection checks, that projected non-occluded visible markers have marker-colored pixels near their screen-space positions, that projected and non-occluded terrain/foliage/cloud/distant-island/wind scene samples have matching material-like pixels near their screen-space positions, and that screenshots pass a lightweight visual audit for resolution, nonblack/nonwhite exposure, luma variance, color variety, edge density, per-frame scene coverage, per-frame center detail, scene detail tile frequency, flat low-detail scene-tile dominance, dominant low-detail scene-component dominance, player visibility, severe border clipping, non-opaque PNG alpha, large foreign bright-canvas regions, HUD-text dominance, route-marker readability/component identity/hue diversity, distant horizon/impostor component readability/color-bucket/span identity, terrain/material family diversity plus terrain material coverage/color/tile spread, foliage coverage/tile spread, cloud-layer coverage/component/span identity, and top-sky coverage across the final screenshot plus fixed checkpoints. The audit catches gross render, composition, transparency, baked desktop/window canvas, low-frequency scene-detail failures, large smooth primitive-like scene surfaces, collapsed marker hue identity, missing or flat distant horizon silhouettes, one-family or missing/flat terrain material screenshots, missing or overly clustered readable foliage coverage, and missing or collapsed cloud-layer image readability. Marker sidecars add exact known-marker viewport projection plus approximate terrain-occlusion classification for route beacons, route objectives, uncollected aerial power-ups, and semantic terrain/foliage/cloud/distant-island/wind sample points; `terrain_surface` samples also carry terrain material variant identity for projected diversity checks, and the marker and semantic scene pixel audits ignore samples classified as occluded/offscreen/behind-camera. Wind-critical checkpoint sidecars must retain at least one visible wind sample so wind-current screenshots cannot pass with the airflow cues offscreen, while late non-wind checkpoints can still be validated by the aggregate semantic scene audit. The semantic scene audit reports per-checkpoint in-viewport, occluded, and visible scene-sample counts, per-checkpoint visible material families, per-checkpoint visible scene sample kinds, per-family and per-kind visible sample counts, terrain material/biome variant counts for visible `terrain_surface` samples, per-family and per-kind visible sample counts and pixel-hit counts, minimum hit floors, hit ratios, tolerant per-visible-variant pixel-coverage floors, conditional wind visual hit checks, and aggregate pixel-coverage checks for each expected material family and scene kind; it fails when any visible material family lacks enough material-like pixels, when a checkpoint has too few distinct visible scene sample kinds, when terrain material/biome variants do not clear aggregate diversity floors, when too few visible terrain material variants have matching variant-class terrain pixels, when matching variant hits are nearly absent instead of readable projected terrain, when any expected material family or sample kind has no visible sample/hit across the checkpoint sequence, or when terrain/foliage/cloud/distant-island projected hits are too small to clear their aggregate coverage floors. These audits use tolerant per-variant terrain color classifiers for `terrain_surface` samples rather than exact per-pixel material ownership; they are projected-sample diversity gates, not production material classifiers, and do not prove exact 3D occlusion, exact per-pixel world semantics, or AAA-quality art direction.
`asset_fixture_audit` checks every declared glTF fixture for self-authored provenance, `extras.nau` registry metadata (`nau_visual_asset_fixture.v1`, asset kind, label, residency, and self-authored license contract), semantic component names, mesh/material/vertex/triangle floors, normals, UVs, blend-material expectations for transparent fixtures, and the player named animation clip inventory. It also rejects reused authored idle/walk/run, bank, or fall clip motion so grounded locomotion, directional banking, and falling cannot pass by aliasing nearby tracks. The current player-fixture contract requires semantic hand, finger, toe, bridge, sleeve, yoke, collarbone, pelvis, deltoid, knuckle, sole, heel, seamless, flex, upper-arm ridge, forearm ridge, thigh panel, lower-leg ridge/tendon, and sculpted head detail pieces including brow, nose, jaw, cheek, temple, and eye-glint nodes in addition to face/eye/belt/gauntlet/knee details, gates finger-grip and boot-sole silhouette proportions, gates wrist-aligned hand pivot nodes with distal palm mesh children, gates launch takeout overhead-arm reach, gates mesh-projected fall/glide/dive silhouette spans from the actual generated glTF hulls, gates high-density organic upper-arm/forearm/thigh/lower-leg/boot mesh volume floors, gates denser reusable hand-palm/finger/fingertip/toe/sole/heel/web/instep/tongue mesh volume floors, gates head-detail node/mesh count and mesh-density floors, gates a 12-node moving bridge-sleeve layer with 24 socket-plus-distal surface contacts plus a 12-node moving flex-cover seam layer across shoulder/elbow/wrist/hip/knee/ankle joints, gates a 22-pair major-joint deformation envelope for shoulder/elbow/wrist/hip/knee flex covers, samples direct mesh-surface connector distances across grounded idle, grounded walk, grounded run, launch, fall, glide, dive, air-brake, landing anticipation, landing recovery, and blended traversal poses, treats shallow mesh overlap as connector contact for the gap metric while separately bounding joint-cover, bridge-sleeve, and flex-cover clipping through overlap gates, reports rest-pose non-adjacent and shoulder worst-overlap pairs for review triage, and transition-sweeps runtime-style blends between launch, fall, glide, dive, air-brake, landing anticipation, and landing recovery poses so fixture gaps, marker-like hands, collapsed fall/glide/dive outlines, and clipping are checked between settled key poses. The current world-fixture contract requires richer semantic fragments for terrain erosion/path detail, foliage root/fern/moss detail, water lily/specular detail, rock rust/shale detail, route glyph/pebble detail, cloud haze/filament detail, and distant waterfall/broken-cliff impostor detail. It is still a structural fixture gate, not a substitute for production art review.
...
- `pose_landing_foot_split_m`
- `pose_landing_recovery_flip_degrees`
- `pose_wing_airflow_strength`
- `pose_scarf_stream_m`
- `pose_scarf_lateral_sway_m`
- `pose_scarf_tail_flex_degrees`
...
- `terrain_body_collision_resolved_count`
...
- `authored_transition_from_clip_label`
- `authored_transition_to_clip_label`
- `authored_transition_active`
- `authored_transition_elapsed_ms`
- `authored_transition_progress`
- `authored_transition_class_label`
...
The visual asset fields report the declared glTF scene inventory, how many slots are loaded and ready, how many are still using generated placeholders, how many Bevy scene handles are deferred/queued/loading/loaded/failed, how many optional scene instances have spawned/reported ready, how many non-player authored world fixture kinds are visibly placed in the scene, how many named player animation clips are declared/ready, whether nested animation players were linked, whether animation graphs were prepared, and how slots divide across always-loaded, stream-window, near-LOD, far-LOD, and weather residency classes. The authored player animation fields report runtime current/desired clip labels, linked authored player count, pair-aware transition duration/class/from/to/progress, and whether the sample is still inside a blend so clip-selection regressions and transition pops are inspectable in sample artifacts instead of only through visible pose scoring; summary metrics count only settled authored clip matches/mismatches plus launch, glide, bank-left, bank-right, fall, dive, air-brake, and landing clip coverage. Runtime visible-pose metrics count all visible articulated authored nodes, not only the torso/arm/leg coarse controls, then report temporal deltas, minimum limb clearance, limb penetration, and marker-to-joint gaps; app checks gate `max_visible_pose_part_count` so the full authored articulation set cannot disappear while coarse readability still passes. Authored glider fields report the largest visible glider rotation response and local motion offset from its base placement, so the app eval can fail if the visible glider stays static while body/pose math passes. Missing files, deferred admissions, and failed loads count as placeholder-backed; deferred slots do not allocate Bevy handles, and queued/loading files are not counted as ready until Bevy reports them loaded. These are readiness signals for replacing primitives with real assets; they now gate every declared fixture slot, zero deferred current fixtures, the seven visible non-player world fixture kinds, the self-authored player fixture's full named clip set including `Fall_Loop` and directional bank clips, distinct authored launch/glide/land/fall/bank clip motion instead of clip reuse, one linked `AnimationPlayer`, and one ready `AnimationGraph`. The separate asset fixture audit verifies fixture semantic-name, geometry, and provenance floors; its player section emits `player_pose_contact_audit`, a pose-by-pose contact report for grounded idle, grounded walk, grounded run, launch, fall, glide, dive, air-brake, landing anticipation, and landing recovery across eight phase samples, including per-pose worst articulated-joint gaps, joint-cover gaps/overlaps, non-adjacent mesh overlaps, and breach count. It also emits `player_pose_transition_contact_audit`, which samples nine high-risk pose transitions at four blend weights and the same eight animation phases, reports worst articulated-joint gaps, joint-cover gaps/overlaps, bridge-sleeve gaps/overlaps, non-adjacent overlaps, and breach count, and fails when in-between frames violate the settled-pose contact thresholds. It also emits `player_hand_pivot_wiring_audit`, which requires left/right hand pivot nodes to stay wrist-aligned while their palm meshes live as distal children instead of using the visible palm as the rotation pivot; `player_joint_bridge_contact_audit`, which reports 12 expected moving bridge-sleeve nodes, 24 socket-plus-distal surface contacts, per-pair worst bridge gap and overlap, overall max gap/overlap, and breach count across the sampled poses/phases; `player_joint_seam_contact_audit`, which reports 12 moving flex-cover seam nodes, 26 seam/contact pairs, pose and transition sample counts, per-pair worst seam gap, per-pair minimum seam overlap, per-pair maximum seam overlap, and breach count; `player_joint_deformation_envelope_audit`, which derives a 22-pair shoulder/elbow/wrist/hip/knee flex-cover envelope from the seam audit and fails over-thin, disconnected, or over-engulfing covers; `player_proximal_contact_audit`, which reports expanded shoulder/arm/forearm/hip/thigh/knee/lower-leg connector gaps and bounded proximal mesh overlap; `player_pose_surface_contact_audit`, which reports sampled mesh-surface distance plus projected visual gap so intersecting boxes cannot automatically pass as connected surfaces; `player_motion_integrity_overlay_warning_audit`, which fails if the large human-review panels would need overlap warning overlays; and `player_mesh_silhouette_audit`, which projects the current generated player mesh hulls across front/rear/side/top pose previews and gates the fall/glide wide outline plus streamlined dive narrowing. The pose-preview manifest also includes player/glider source byte counts, stable fixture hashes, front/rear/side/top view coverage, motion-integrity panel counts, overlay warning/fail counts, and the same mesh silhouette audit so human-review SVGs can be tied back to exact fixture inputs. These signals still do not prove final art quality yet.
...
The environment-motion fields report how many resident near-LOD visuals are wind-responsive and the largest sampled transform offset from their base placement. Dynamic wind-flow fields report how many visual wind volumes contain the sampled player position, the strongest shared flow speed, the strongest gust variation, and the largest local/temporal flow-direction change sampled from the same shared `WindField` catalog used by updraft and crosswind guide visuals. Shared `WindField::flow_at` currents include soft edge falloff, spatial gust cells, moving gust-front progress, clumped packet strength, secondary lane-dependent gust strength, and depth-layered crosswind shear, so adjacent lanes and heights can move with different speed, direction, lift, and swirl while preserving horizontal crosswind flow and upward-biased updraft flow. Wind-force fields report the bounded horizontal airborne response from those visual `WindField`s, including total applied delta, crosswind-only delta, source flow speed/variation, force-to-flow correction alignment against the field-axis direction needed to move current speed toward sampled flow speed, and the delta component aligned with that correction direction; updraft `WindField` swirl can add lateral current, while paired vertical lift still comes from `LiftField` but scales its acceleration from the matched visual updraft's thermal core, gust strength, soft edge, and sampled upward flow before applying the `LiftField` speed cap. Projected crosswind-drift fields measure neutral-input horizontal displacement along the dominant horizontal axis of the active crosswind force, so routes prove perceptible sideways mechanical wind without letting normal forward glide travel or small down-route shear satisfy the drift floor. Dynamic lift fields now report whether active lift samples also had paired visual updraft flow, whether the matched updraft flow changed the lift multiplier, the strongest applied vertical delta, and the sampled multiplier range so a route cannot pass by merely overlapping a static `LiftField`. Layered wind-force summary fields count samples where at least two wind fields affect the player at once, samples where crosswind and updraft swirl overlap in the same frame, aligned overlap samples, max layered field count, and layered-only delta/alignment floors; `updraft_route` gates those values so a single isolated updraft or a route that encounters crosswind only outside lift cannot pass. Wind-load response fields count neutral airborne/gliding samples where gust-varied shared `WindField` flow and total applied wind-current delta create a body-local lateral load, then gate max normalized load, pose lean, and authored glider response so wind cannot affect velocity while leaving Nau and the glider visually inert. The wind guide visual fields separately count updraft motes/ribbons and crosswind motes/ribbons, then report per-field coverage for guide presence, ribbon presence, guide-plus-ribbon pairing, and field-level flow coherence; app evals animate wind guide/ribbon/column systems from the deterministic eval frame clock so authored visual-flow metrics, observed transform deltas, movement, and wind-force samples share one timebase. The summary gates require all declared updraft and crosswind fields to have paired/coherent visuals so one strong field cannot hide a missing neighbor. The same fields also report the largest sampled guide/ribbon offset from baseline placement, updraft guide rise, updraft swirl displacement, updraft vertical depth span, crosswind lane/height depth span, baseline-relative guide/ribbon scale pulse, split guide/ribbon crosswind flow-direction displacement, short-horizon visual-flow alignment against the same `WindField::flow_at` source used by gameplay force sampling, and sustained sample counts for updraft/crosswind/all wind visual flow so a single good frame cannot satisfy the airflow surface. Observed wind visual fields additionally compare actual ECS transforms between adjacent eval frames, require both previous and current positions to remain inside the visual field, skip invisible loop-fade frames, reject field-center fallback, gate observed frame-motion counts, displacement floors, flow-alignment floors, max visual speed, max acceleration, and visible wrap/jump count for updraft guides/ribbons, crosswind guides/ribbons, and crosswind ribbon scene samples. Broad crosswind ribbons additionally sample front, center, and tail scene positions after field-direction advection, re-query local `WindField::flow_at` at those scene positions without a field-center fallback, and gate both coherent sample count and maximum ribbon-specific flow alignment so a large ribbon cannot pass by sliding as one detached billboard while its visible body ignores the local current. Updraft ribbons now sample a visible spiral-lane probe inside the updraft column before pulsing and drifting, so their gust response follows local height/radius layers instead of a detached field-center sample. Sustained visual-flow counts require each counted sample to clear 75% of the same visual count, motion, depth, pulse, and coherence floors plus full flow-alignment floors, observed frame-motion floors, and a separate sustained ribbon-advected-flow window requires repeated local ribbon scene-sample coherence, so weak nonzero motion cannot pad the sustained windows. Focused visual tests cover stream-specific crosswind motion variation, shared gust-packet fronts, layered gust energy between primary fronts, observed frozen/off-field visual rejection, local updraft ribbon thermal-lane sampling, ribbon-specific advected-flow sampling, visual-quality speed/acceleration/jump rejection, and crosswind drift projection so guides/ribbons do not move in lockstep and visual pulses do not drift away from gameplay flow. This keeps missing, static, flat, one-layer-only, lockstep, wrong-axis, one-good-frame, one-good-field, velocity-only, billboard-only, teleporting, or gameplay-detached airflow cues from hiding behind generic tree/cloud sway or lateral jitter. Screenshot sidecars also project wind guide/ribbon samples and require visible wind samples on wind-critical routes. Summary metrics track the sampled direction-change floor and variation range while lift is active/readable, so nonzero but static wind-flow values cannot satisfy lift-required scenarios. Together they prove the visual motion layer, readable lift flow, first gameplay wind-current response, and first body/glider wind-load reaction are active; they do not evaluate final animation quality.
The island-terrain fields report generated terrain surface count, minimum terrain mesh vertex count, minimum vertex-color band count, minimum encoded material-weight band count, minimum material channel count, minimum derived material-region count, minimum terrain texture-detail band count, minimum sampled terrain relief range, route terrain-archetype diversity, and minimum cliff/underside color-band count. The material weights are currently encoded into `UV_1` as lush/highland and exposed-edge blend channels; material regions quantize those channels into stable base, transition, lush, and exposed-edge identities; texture-detail bands count coarse color bins in the terrain-specific procedural albedo maps, which are 512px, world-space tiled, and include sharper pitted strata, fissure, mineral-fleck, and micro-grain detail instead of being stretched once across an island, so future PBR material blending or glTF export has a measurable substrate. The headless terrain export adds terrain-archetype diversity, texture-edge, height-band, and normal-slope-band floors over manifest and OBJ artifacts so repeated island grammar, smeared, flat, or single-slope terrain fills fail offline even before screenshot review. Focused world tests also guard large traversible anchor-island footprint area plus midfield relief complexity from braided path cuts, raised berms, and strata/crag reversals. These are structural signals for denser generated island substrate with larger anchor surfaces, ravines, terraces, basins, ridges, paths, crags, and microrelief; they do not replace screenshot or human review for final material quality.
...
Current substrate gates require 32 generated island terrain surfaces across low cays, mid-route anchor islands, and high summit islands; all 19 declared route terrain archetypes; at least 32 terrain color bands, 24 terrain material-weight bands, 19 terrain height bands, 10 terrain normal-slope bands, 50 terrain texture-detail bands, 590 terrain texture-edge promille in export audit, 140 island-impostor mesh vertices, 18 island-impostor color bands, 1600 island-body mesh vertices, 55 generated trunks/canopies, 450 canopy mesh vertices, 90 generated rocks, 60 generated landmarks, 16 route cairns, 1 launch beacon, 4 landing-garden markers, 32 pond surfaces, 32 route obstruction spires, 39 landmark mesh vertices, 300 obstruction-spire mesh vertices, 500 obstruction-spire triangles, 3.0m obstruction-spire vertical span, 6 obstruction-spire height bands, 5 obstruction-spire radius bands, 5 obstruction-spire normal-slope bands, 40 generated weather clouds, 32 generated weather cloud banks, 9 weather-cloud lobes, 36 weather-cloud wisp cards, 1530 weather-cloud mesh vertices, 5.8m generated cloud-bank depth, and 27 weather-cloud filament ribbons. Wind-content gates also track five gameplay updrafts and seven paired crosswind fields so the expanded vertical route cannot regress to the old near/far lift pair.
...
- lateral right/left, pure-sideways AirTurn right/left, and backward-diagonal right/left body/travel heading sample counts plus p95/max error
- visible pose part count, explicit aggregate and right/left airborne turn-pose sample counts, max pose lateral lean plus signed right/left and backward-right/backward-left air-brake pose lateral lean, global visible-pose temporal stability sample count, max global pose-part rotation/translation deltas, landing-only temporal stability sample count, max landing pose-part rotation/translation deltas, and max landing forward-fold/backward-bend
...
- minimum camera distance while obstruction avoidance is active
- repeated obstruction snap count
...
- max dynamic wind-flow field count, wind-flow speed, wind-flow variation, wind-flow direction change, and readable-lift variation range
- wind-force sample counts, sustained meaningful and flow-aligned wind-force sample counts, crosswind/updraft-swirl/layered force sample counts, crosswind-updraft overlap sample counts, max active wind-force field count, crosswind/updraft-swirl/layered force field counts, wind-force delta (total applied wind-current delta), crosswind delta, updraft-swirl delta, layered delta, wind-force flow speed, wind-force variation, force-flow alignment, layered force-flow alignment, and flow-aligned delta
- wind-load response sample count coupled to gust-varied shared `WindField` flow and total applied wind-current delta, plus max body-local lateral load, wind-driven pose lean, and wind-driven glider response
...
- max updraft/crosswind short-horizon flow-coherent visual counts and flow alignment, plus ribbon-specific advected crosswind flow-coherent sample count and max alignment
- max observed updraft/crosswind visual speed, max observed wind visual acceleration, visible wind visual jump count, and projected neutral-input crosswind drift/step
...
- max terrain-body collision proxy count
...
- terrain-body collision resolved sample count
- terrain-body collision meaningful-contact sample count
- max world-collision push distance
- max terrain-rim collision push distance
- max terrain-body collision push distance
...
- dynamic readable lift sample count plus max sampled wind-flow speed, gust variation, flow-direction change, and readable-lift variation range
- dynamic lift application sample count (`dynamic_lift_samples`) plus `max_paired_visual_lift_fields`, `max_dynamic_lift_fields`, `max_lift_applied_delta_mps`, `min_dynamic_lift_multiplier`, `max_dynamic_lift_multiplier`, and `max_dynamic_lift_multiplier_range`
- max launch upward and horizontal speed while the player is in `Launching`
- pose-intent sample counts for readable gliding, launching, falling, diving, air brake, landing anticipation, and landing recovery; app and simulation summaries also count grounded idle/walk/run pose samples, deployed-glider dive samples, right/left air-turn samples, same-sample pure-sideways AirTurn alignment samples, right/left plus backward-right/backward-left air-brake pose samples, and zero gliding landing-anticipation samples, pose-state checks require both right and left air-turn samples, and app evals score visible generated or authored player part transforms when those nodes are available and still require authored clip alignment when authored player geometry hides generated fallback parts
- pose torso pitch, arm spread, leg tuck, dive-only torso pitch, dive-only arm spread, dive-only leg tuck, lateral lean, signed right/left lateral lean, signed backward-right/backward-left air-brake lateral lean, landing crouch, landing foot-forward tuck, upper-leg landing foot split, distal lower-leg/foot landing split, max landing foot split, signed landing forward-fold, bounded positive landing backward bend, wing-airflow maxima, scarf stream/sway/flex maxima, authored glider response/motion maxima, authored glider dive response/motion maxima, authored idle/walk/run/bank-left/bank-right/fall/dive/air-brake/landing clip sample counts, key-pose readability min/max, unreadable key-pose sample count, and key-pose transition-grace sample count, including bounded app grace for short air-brake, glide, dive, landing-absorb, and landing-anticipation release blends; focused pose unit tests also require dive-specific phase-staggered airflow motion across torso, head, arms, and legs while preserving key-pose readability
...
- launching samples stay under the simulation caps for upward and horizontal launch speed, so the launch remains a modest vertical assist instead of a route-completing burst
...
- lift-required scenarios sample non-static dynamic wind flow with enough speed, gust variation, local/temporal direction change, and variation range while lift is active/readable
- lift-required simulation scenarios must also register actual dynamic lift application from paired visual updraft flow, clear an applied vertical delta floor, and clear a lift multiplier range floor
...
- `updraft_route` must register wind-load reaction samples coupled to gust-varied shared `WindField` flow and total applied wind-current delta and clear normalized lateral-load, pose-lean, and glider-response floors so force-only wind does not pass
- the app scene contains enough updraft and crosswind guide/ribbon visuals; the current minimums are 126 updraft guide motes, 12 updraft ribbons, 180 crosswind guide motes, and 21 crosswind ribbons
- wind guide/ribbon visuals animate enough to prove the sampled airflow cues are not static, including minimum updraft vertical depth span, updraft scale pulse, crosswind lane/height depth span, and crosswind scale pulse tied to shared gust-packet/layered-gust flow samples
...
- enough wind guide/ribbon visuals must have short-horizon motion aligned with the shared `WindField::flow_at` vector; current floors require 126 coherent updraft visuals, 180 coherent crosswind visuals, at least 30 advected crosswind ribbon flow-coherent scene samples, and at least 0.55 max alignment for each family; the offline wind export audit also gates updraft-ribbon coherent tracks and its family-specific low-alignment ratio so thermal ribbon motion cannot be masked by the aggregate wind-track ratio
- observed wind visual speed and acceleration must stay below the configured quality ceilings, visible wrap/jump count must remain zero, and neutral-input crosswind drift must clear a projected displacement floor while staying under the per-step drift ceiling
- crosswind ribbon scene-sample coherence must also sustain for at least 28 samples so broad ribbons cannot satisfy the route with one isolated aligned frame or guide-only motion
...
- the island-visual catalog unit audit classifies every generated solid visual name by collision kind, requires 16 sampled terrain-rim contour segments and four terrain-body cliff panels per route island, and fails any camera obstacle without collision unless it is explicitly allowlisted as camera-only foliage
...
- terrain-rim collision proxy count stays populated so near-LOD sampled island-edge contour segments cannot silently disappear
...
- the `terrain_body_collision_contact` app route starts in a clean launch-mesa east-cliff lane, sustains terrain-body collision samples above a 5 mm per-sample push floor, reaches a nontrivial peak terrain-body push while taxiing into the visible launch-mesa cliff body, and requires zero terrain-rim resolved samples, so aggregate proxy count and invisible rim contact cannot mask missing body collision
- `ground_taxi_control` records zero terrain-rim resolved samples, so normal pre-launch WASD movement cannot pass while hidden rim contour segments scrape or snag the player
- island terrain surface count, terrain-archetype count, mesh vertex floor, vertex-color band floor, material-weight band/channel/region floors, texture-detail floor, relief-range floor, height-band/normal-slope-band floors, and cliff color-band floor stay populated, while focused world tests require larger traversible anchor footprints and nontrivial midfield relief reversals, so the generated world cannot silently regress to repeated island grammar, smaller-feeling route surfaces, lower-resolution, blurrier, flatter, single-slope, or visually single-tone island surfaces
- terrain export additionally gates near/mid/macro texture detail energy, near-to-mid energy ratio, isolated-edge ceilings, material-region parity, terrain/cliff seam continuity, and runtime/export `SurfaceMaterial` parity so a sharp but noisy texture, disconnected skirt, or export-only fallback cannot pass
...
- stream visibility changes per frame stay under budget, and the runtime caps initialized island stream spawn/despawn work to the same per-frame budget so chunk/LOD crossings do not churn too many visuals at once
...
- camera obstruction avoidance was exercised when a scenario requires it; every app sample separately reports collision-only vertical obstruction correction and stays within the `0.241 m` 60 Hz budget, `camera_mouse_control` must include simultaneous input attribution plus route-spire obstruction hits so that bound cannot pass vacuously, and `world_collision_contact` exercises generated launch-mesa tree/prop blockers with minimum readable obstructed camera distance plus zero repeated obstruction snaps so standalone test cuboids cannot be the only obstruction coverage
- camera mouse scenarios exercised yaw and both pitch directions
- air-control response latency, right/left/rear-right/rear-left lateral response and latency, rear-right/rear-left rearward response, pure-backward body-heading intent, body-bank response, body-roll step, total and planar air-brake speed drop, readable right/left air-turn, same-sample pure-sideways AirTurn body/travel plus desired-travel alignment, right/left/backward-right/backward-left air-brake, and deployed-glider dive pose coverage, authored bank-left/bank-right/dive/air-brake clip coverage, pose torso pitch at least 45 degrees, arm spread at least 100 degrees, leg tuck at least 35 degrees, dive-only torso pitch at least 60 degrees, dive-only arm spread at least 165 degrees, dive-only leg tuck at least 55 degrees, dive head-gaze down alignment in the readable `0.60..=0.94` band including dive-entry transitions, lateral lean at least 8 degrees, signed right/left pose lean at least 8 degrees in the matching input direction, signed backward-right/backward-left air-brake pose lean at least 8 degrees in the matching input direction, dive key-pose readability whose 0.9 floor corresponds to at least a 60-degree torso pitch and 165-degree arms-out silhouette, wing-airflow strength at least 0.25, scarf stream at least 0.25 m, scarf lateral sway at least 0.03 m, scarf tail flex at least 12 degrees, visible authored glider response at least 4 degrees, dive-specific authored glider response at least 4 degrees, dive-specific authored glider motion at least 0.04 m, zero key-pose samples below the 0.9 readability floor, global visible-pose temporal samples and bounded global pose-part deltas, post-brake forward alignment, desired-heading and aggregate plus right/left/backward-right/backward-left desired-travel alignment, average/p95/max body-heading error, nonzero right/left pure-sideways AirTurn, right/left lateral, and backward-right/backward-left diagonal sample counts plus p95/max body/travel heading error, max yaw-error step, yaw oscillation count, camera orbit yaw offset, camera view-yaw drift, camera world-yaw drift, and camera rotation delta stayed inside thresholds
- pose-state coverage scenarios record grounded idle/walk/run samples, grounded walk/run stride foot travel and leg opposition, readable launch/fall/glide/air-turn/air-brake/gliding-dive samples with dive-only torso/arm/leg silhouette floors and bounded dive head-gaze down alignment, right and left air-turn pose samples, settled same-route pure-sideways right/left AirTurn samples with p95/max body/travel and desired-travel heading error, backward-right/backward-left diagonal air-brake samples with matching signed air-brake lean, landing anticipation/recovery samples, zero gliding landing-anticipation samples, landing crouch, feet-forward tuck, asymmetric foot-split floor plus bounded max foot split, signed landing forward-fold and backward-bend bounds, bounded landing local torso bend and torso offset, app landing-only pose temporal deltas, scarf stream/sway/flex floors, zero upward-velocity fall poses, zero dive poses without dive input, and app-authored `idle`/`walk`/`run`/`launch`/`glide`/`bank_left`/`bank_right`/`fall`/`dive`/`air_brake`/`land` clip samples while rejecting unreadable key-pose samples
...
- landing-required routes exercised readable airborne-only landing-anticipation pose coverage before contact, rejected landing anticipation while still gliding, readable landing-recovery pose coverage after contact, reached at least 0.05 m of landing crouch, reached at least 0.32 m of feet-forward landing tuck, kept landing foot split below the readability ceiling, reached signed forward-fold absorption while keeping positive backward bend under threshold, kept landing local torso bend and torso offset under their thresholds including transition-grace frames, produced zero key-pose samples below the 0.9 readability floor after bounded transition grace, recorded landing-only visible-pose temporal samples, and kept landing pose-part rotation/translation deltas within landing-specific route thresholds
...
- `terrain_body_collision_contact`: current app-only visible island body collision contact regression test.
...
- `air_control_response`: current diagonal/lateral/rear-right/rear-left/brake/recovery air-control response test with same-sample pure-sideways AirTurn alignment and a bounded transition-grace budget for ambiguous airborne key-pose blends.
- `pose_state_coverage`: current full-pose chain coverage for idle hold, walk/run stride, launch/fall/glide, right and left air-turn, settled pure-sideways right/left AirTurn alignment, backward-right/backward-left diagonal air-brake, air-brake/gliding-dive, landing anticipation/recovery shape, app-authored idle/walk/run/launch/glide/bank-left/bank-right/fall/dive/air-brake/land, zero-unreadable pose-state coverage, and bounded transition-grace samples.
- `camera_stress`: future dedicated close-geometry route that records camera distance, pitch, obstruction metrics, player-framing distance, and fixed checkpoint screenshots beyond the current generated tree/prop coverage in `world_collision_contact`.
- `wind_visual_motion_quality`: current wind routes or export audit extended to gate visual speed, acceleration, wrap/jump artifacts, visible-flow alignment, and perceptible-but-bounded horizontal crosswind drift.
- `player_pose_fidelity`: current pose routes or a focused visual sidecar route extended to gate limb-spacing/self-intersection risk, fall belly-down orientation, dive head-down streamlining, transition smoothness, and silhouette continuity.
...
- Metric-only app evals skip screenshot capture, but still instantiate the native Bevy window/rendering stack and report wind-current metrics; use `NAU_EVAL_SIM_ONLY=1` when the run only needs windowless traversal/camera/wind math.
...
- The current wind eval surface proves shared-field coherence, non-static motion, observed visual speed/acceleration/jump quality, projected neutral-input horizontal crosswind drift, and bounded horizontal current. It still does not prove final airflow art direction, particle/cloth richness, or subjective feel; those need screenshot/video review plus future art-specific metrics.
- The current pose eval surface proves named state coverage, readable key-pose floors, sampled connector surface distance, projected visual contact gaps, proximal shoulder/hip contact bounds, wrist-pivoted hand wiring, major-joint deformation-envelope bounds, organic major-limb volume density floors, reusable extremity mesh-density floors, fingertip-aware player/glider hand-to-grip attachment gates, a minimum limb-anatomy detail layer, rest-pose worst-overlap reporting, and mesh-projected full-body fall/glide/dive silhouette gates. It still does not prove final character art quality; future passes should add temporal hand-grip video review, pose-smoothness review, and richer rendered sidecars before substantially increasing model complexity.
- Camera obstruction checks now prove avoidance, bounded step/rotation, separately measured collision-only vertical correction capped at `0.241 m` per 60 Hz app frame, non-vacuous simultaneous input plus obstruction coverage, minimum readable obstructed camera distance, and zero repeated obstruction snaps on the shared route-spire route plus the generated launch-mesa tree/prop contact route. Semantic island-ray checks use authored visual footprints instead of coarse ellipses, but their vertical volume remains approximate. Exact occlusion quality, final composition, and art-direction review still need human/agent screenshot inspection before treating the camera as production-final.
- Surface-specific screenshot coverage separates muted terrain from water and foliage, rejects shadow-flattened terrain tiles, and requires coherent local connected components for ponds, channels, waterfall water, plunge pools, and waterfall foam. Landscape-vista relaxations are only enabled when semantic sidecars prove the omitted route-marker or coarse-foliage evidence, so a scenic composition cannot silently bypass material identity.
...
- Release baseline and play-profile frame-time metrics skip warmup and remain local native-window trend evidence rather than portable cross-machine truth. The development-play eval records its first 30 frames as startup evidence because deferred stream reconciliation occurs around frame 9, then requires at least 120 runtime samples and gates only the steady runtime average/p95/max/slow-frame metrics. The all-frame maximum remains in the report, so startup work is visible rather than discarded. The gate uses a hidden metric-only native window, runs a separate process warmup, and refuses timing evidence from a busy local host before enforcing a maximum 12 ms runtime average, 18 ms runtime p95, 35 ms runtime single frame, at most 24 runtime frames over 16.67 ms, and at most 128 materials. Visible presentation diagnostics remain focus/compositor-sensitive and do not replace focused human release play. `macos-latest` uses calibrated 70/90 ms average/p95 ceilings plus noise-tolerant 500 ms/201-frame overrides and the shared 1.25 same-host debug/release ratio, while the hardware-independent 116-material cap remains the cross-machine regression guard.
...
- LOD buckets drive resident island detail, inactive or non-near chunks use cheap impostors, hidden/resident/churn counters quantify stream-window pressure, and initialized island streaming applies the churn cap over multiple frames rather than spawning/despawning an entire chunk transition in one frame.
...
- Route objectives are HUD/debug state backed by pure route objective helpers and serialized into eval samples; main routes use the near updraft, branch targets add the recovery updraft, and high/far named targets add deeper upper-thermal, stratos, and summit lift nodes based on target height/depth. Only updraft and branch-recovery routes currently gate objective completion in app/sim scenarios.
