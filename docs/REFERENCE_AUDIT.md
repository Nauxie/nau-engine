...
- Treat lateral air control as a desired-heading problem first, then a velocity problem. NAU should keep the current evaluated camera controller, derive a flat camera-relative desired direction from WASD/left stick, steer planar velocity toward that vector with acceleration curves, and slerp/bank Nau's body toward that same vector without writing to camera yaw. Current follow-through keeps the custom controller, adds smoothed bank state, reports body-roll metrics, and keeps movement input decoupled from camera yaw.
...
NAU follow-through from this audit now uses the same broad shape without adding `bevy_generative` as a dependency: deterministic mesh generation writes positions, indices, normals, tiled terrain UVs, vertex colors, and encoded `UV_1` material weights in one pass; island surfaces have denser radial topology with route-aligned ravine/channel incisions, terrace/shelf variation, and fine microrelief held inside existing collision rise/drop bounds; cliff/underside meshes carry measurable strata/color bands; distant island impostors use layered generated geometry and vertex-color bands instead of single low-detail blobs; terrain materials use sharper 512px generated PBR maps with smoothed broad value-noise variation, pitted strata, fissures, and mineral flecks instead of block-stepped color patches; generated tree trunks now use multi-ring taper, branch mass, and root flares while canopy and cloud meshes add organic detail-card silhouettes on top of lobed primitives; eval gates track terrain surface count, mesh vertex floors, vertex-color bands, material-weight bands/channels/regions, texture-detail bands, terrain texture-edge frequency, relief range, cliff color-band floors, terrain/body/impostor shape floors, island-body mesh density, and generated vegetation/cloud shape floors; `--export-terrain` plus `terrain_export_audit` emit and validate terrain/cliff/underside/impostor OBJ, terrain-material CSV, and manifest artifacts, including height-band and normal-slope-band floors from both manifest and OBJ data, for offline terrain inspection; and `--export-visual-content` plus `visual_content_audit` emit and validate generated ground-cover/tree/cloud OBJ artifacts, multi-ring trunk/branch/root-flare/canopy/cloud-wisp/depth metrics, and biome detail-palette diversity without opening the native app. This improves the generated island substrate while remaining placeholder art rather than production AAA assets.

The July 2026 surface follow-through adds a narrow Bevy `ExtendedMaterial` rather than a custom render pipeline: coherent mipped terrain albedo/normal/ORM maps, `UV_1` terrain masks, generated crossing-wave normals, tangent-aware water orientation, channel/waterfall flow masks, and explicit foam behavior share one shader type. Runtime and export use the same constructors, material accounting includes standard and extended collections, and visual/semantic audits now gate terrain shadow detail plus coherent pond/channel/waterfall evidence. This is a measured generated-art upgrade, not evidence that imported production surface assets are no longer needed.
...
2. Movement follow-through: keep tightening the dedicated diagonal/lateral airborne steering route so it proves desired heading, body heading, body bank/roll response, planar velocity, braking, and camera yaw remain decoupled under mixed WASD input. The reference target is `bevy_new_3d_rpg`'s flat camera-relative movement helper, adapted to NAU's flight physics rather than copied as a ground controller.
...
5. Extend the `bevy_new_3d_rpg` asset/animation shape in stages: NAU now has declared glTF slots, self-authored player/glider/world fixture scenes, registry-aligned `extras.nau` metadata, `SceneRoot` spawning, scene-instance readiness, named player clip declarations, `Gltf` lookup, retryable `AnimationPlayer` discovery, and `AnimationGraph`/`AnimationTransitions` readiness metrics. The current world fixture floor includes terrain erosion/path detail, rooted foliage/fern/moss cards, water lily/specular layers, rock rust/shale details, route glyphs/pebbles, cloud haze/filaments, and distant waterfall/broken-cliff impostor details; the next asset branch should move from these deepened fixtures toward compatible production-quality character/world assets before driving real state transitions.
6. Keep the current surface extension narrow. Broader Foxtrot-style render infrastructure, TAA/HDR changes, extra render passes, or explicit shader-prewarm scenes still require a measured gap that the current Bevy material path and startup/runtime timing split cannot explain.
7. Continue the internal island mesh generator inspired by `bevy_generative`, but preserve the accepted terrain masks, seam continuity, collision truth, water orientation, material budget, and runtime/export parity. The next terrain branch should target authored biome assets or a clearly measured geometric limitation rather than adding more unbounded procedural layers.
8. Extend visual evals only where current terrain/water/foliage/cloud classifiers and semantic sidecars leave a demonstrated blind spot; imported production assets will need stricter ownership and temporal surface checks than the current generated-art tolerance.
