...
- `semantic_scene_audit.json`: projected terrain/foliage/cloud/distant-island material-family and `terrain_surface`/`tree_canopy`/`weather_cloud`/`distant_island` scene-kind hit, terrain material/biome variant diversity, and aggregate pixel-coverage audit for screenshot evals that emit marker sidecars.
