...
The July 22 microstutter follow-up removes additional render and steady-frame CPU amplification: transparent weather and wind visuals now have explicit distance ranges, far island impostors no longer execute the full terrain shader, island residency work is cached once per island, streaming collections reuse scratch storage, and settled world-floor windows skip redundant reconciliation. Play-profile schema v5 also reads the native current monitor, uses the highest observed refresh rate for pacing, and rejects runs whose refresh rate changes or cannot be observed instead of allowing a 120-to-60 Hz transition to produce a false pass. Profiling automation no longer requests foreground focus. These changes still require a user-controlled manual release-play A/B before the residual microstutter can be considered resolved.
